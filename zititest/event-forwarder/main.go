...
// forwardWriteTimeout bounds a single write to the destination service. Edge conns
// default to an effectively infinite timeout, so without this a half-open circuit
// would park the forward loop holding f.mu, taking the keepalive that would have
// noticed the dead connection down with it.
...
type zitiForwarder struct {
	...
	// dial opens a connection to the destination service, and writeTimeout bounds a
	// single write. Both are fields so tests can drive the reconnect path without
	// standing up a Ziti context. Zero writeTimeout means forwardWriteTimeout.
	dial         func() (net.Conn, error)
	writeTimeout time.Duration
}

// dialDestination opens a connection to the destination service.
func (f *zitiForwarder) dialDestination() (net.Conn, error) {
	if f.dial != nil {
		return f.dial()
	}
	return f.ctx.Dial(f.service)
}

// writeDeadline returns the deadline to apply to the next write.
func (f *zitiForwarder) writeDeadline() time.Time {
	timeout := f.writeTimeout
	if timeout == 0 {
		timeout = forwardWriteTimeout
	}
...
func (f *zitiForwarder) runKeepalive(conn net.Conn) {
	...
	for range ticker.C {
		...
		err := conn.SetWriteDeadline(f.writeDeadline())
		if err == nil {
...
// forward writes line to the destination, reconnecting and retrying until it
// succeeds or the forwarder is closed. Each write is bounded by
// forwardWriteTimeout; a timeout is treated like any other write failure, so a
// half-open circuit is discarded and redialed rather than parking here forever.
func (f *zitiForwarder) forward(line string) error {
	...
	for {
		// Checked every attempt, not just on the dial path in ensureConnected, so a
		// destination that accepts connections but never reads still lets us stop.
		select {
		case <-f.done:
			return errShutdown
		default:
		...
		err := f.conn.SetWriteDeadline(f.writeDeadline())
		if err == nil {
			_, err = f.conn.Write(data)
		}

		if err != nil {
			log.WithError(err).Warn("write failed, reconnecting")
			conn := f.conn
			f.conn = nil
			conn.Close()
