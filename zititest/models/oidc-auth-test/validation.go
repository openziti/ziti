...
// expectedDialTargets enumerates the component IDs this test expects to see
// healthy traffic reported for. Each ziti-prox-c is a proxy target dialed by
// the csdk traffic driver on its host; each go-client runs its own sdk-direct
// driver that reports under its own ID. Traffic events carry ClientId equal to
// the target's component ID, so this set is exactly the ClientIds we wait for.
func expectedDialTargets(run model.Run) map[string]bool {
	expected := make(map[string]bool)
	for _, c := range run.GetModel().SelectComponents(".prox,.go-client") {
		expected[c.Id] = true
	}
	return expected
}

// quietWindow is how long the test must observe zero errors globally (across
// any target) before declaring convergence, on top of every individual target
// being healthy. This absorbs post-chaos tails where the per-target gate would
// pass at the instant an errored target recovers but further errors might
// still trickle in over the next few seconds.
const quietWindow = 30 * time.Second

// waitForAllTargetsHealthy waits until both conditions hold:
//  1. Every expected dial target's latest event is a success (Healthy()).
//  2. No error event has been observed anywhere in the last quietWindow.
//
// Condition 1 tolerates transient warm-up errors that any individual target
// recovered from. Condition 2 ensures the system has been globally quiet for
// long enough that we aren't racing into cycle 1's strict zero-error window
// while the tail of a disruption is still unspooling.
func waitForAllTargetsHealthy(run model.Run, since time.Time, timeout time.Duration) error {
	log := tui.ValidationLogger()
	expected := expectedDialTargets(run)
	deadline := time.Now().Add(timeout)
	var lastLog time.Time

	for time.Now().Before(deadline) {
		health := trafficCollector.ClientHealthSince(since)
		notHealthy := unhealthyTargets(expected, health)
		recentErrors := trafficCollector.ErrorCount(time.Now().Add(-quietWindow))

		if len(notHealthy) == 0 && recentErrors == 0 {
			log.Infof("all %d dial targets healthy with no errors in last %s",
				len(expected), quietWindow)
			return nil
		}
		if time.Since(lastLog) > 15*time.Second {
			log.Infof("healthy dial targets: %d / %d, errors in last %s: %d, waiting...",
				len(expected)-len(notHealthy), len(expected), quietWindow, recentErrors)
			lastLog = time.Now()
		}
	...
	return fmt.Errorf("after %s: %d of %d targets unhealthy, %d errors in last %s. sample: %v",
		timeout, len(notHealthy), len(expected), recentErrors, quietWindow, sample)
}

func unhealthyTargets(expected map[string]bool, health map[string]zitilibOps.ClientHealth) []string {
	var notHealthy []string
	for id := range expected {
		if !health[id].Healthy() {
			notHealthy = append(notHealthy, id)
		}
	}
	return notHealthy
}
...
func testSteadyState(run model.Run) error {
	...
	// Require every (driver, proxy-slot) pair to have reported successful traffic
	// with zero errors before we enter the strict zero-error cycle loop. This
	// absorbs post-disruption tails where a few proxes lag on session-cache
	// rebuild: the slow slots produce either a later success (coverage) or an
	// error (failure), rather than sliding into cycle 1 as a flake.
	convergenceStart := time.Now()
	if err := waitForAllTargetsHealthy(run, convergenceStart, 15*time.Minute); err != nil {
		return fmt.Errorf("traffic did not converge before steady-state checks: %w", err)
	}

	// Clear so cycle 1's zero-error window starts fresh, decoupled from any
	// warm-up events that may still have trailing timestamps.
	trafficCollector.Clear()
...
func testMediumPartition(run model.Run) error {
	...
	// Refresh tokens outlive a 10 minute partition, so every client must recover
	// through the refresh path. A full re-authentication instead of a refresh
	// means the path under test didn't work, so require refresh events here
	// rather than accepting any authentication event.
	if err := validations.ValidateOidcRefreshed(&eventCollector, clientIdentityIds, healTime, 10*time.Minute); err != nil {
		return fmt.Errorf("clients did not refresh after medium partition heal: %w", err)
	}
...
func testLongPartition(run model.Run) error {
	...
	// Both tokens expire during a 23 minute partition, so the partitioned clients
	// have to establish new sessions. A refresh alone would mean the tokens
	// outlived the partition, which would invalidate the premise of this test.
	if err := validations.ValidateOidcNewSessions(&eventCollector, goClientIdentityIds, healTime, 10*time.Minute); err != nil {
		return fmt.Errorf("Go SDK clients did not fully re-authenticate after long partition heal: %w", err)
	}

	log.Infof("full re-auth recovery took %s", time.Since(healTime))

	// The prox clients were never partitioned, so check the full client set is
	// still connected rather than only the identities that re-authenticated.
	if err := validations.ValidateIdentitiesConnected(run, clientIdentityIds, 5*time.Minute); err != nil {
		return err
	}
