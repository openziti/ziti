...
- `gossip.<type>.delta.received` (meter) - entries arriving from routers or from a peer's broadcast
...
- `gossip.<type>.anti_entropy.owners_diffed` (meter) - owners whose hash mismatched, so their entries were sent
- `gossip.<type>.anti_entropy.entries.received` (meter) - entries arriving in answer to a digest we sent
- `gossip.<type>.anti_entropy.entries.applied` (meter) - repair entries that passed the version check
- `gossip.<type>.anti_entropy.entries.rejected_stale` (meter) - repair entries we already had; expected, since a
  diverged owner is re-sent whole (see the anti-entropy section of `doc/design/gossip.md`)
...
| `gossip.<type>.anti_entropy.entries.received` rate                    | sustained, not just after churn | Peers are perpetually catching up via anti-entropy. The direct-broadcast path is dropping or lagging.           |
| `gossip.<type>.anti_entropy.owners_diffed`                            | > a handful per interval        | More owners diverging than churn accounts for. Each one costs its whole entry set on the wire.                  |
