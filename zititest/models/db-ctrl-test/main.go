...
func (self *dbStrategy) CreateEnrollIdentitiesAction(tx *bbolt.Tx, m *model.Model, builder *models.ZitiDbBuilder) error {
	ids, _, err := builder.GetStores().Identity.QueryIds(tx, "true limit none")
	if err != nil {
		return err
	}

	m.Actions["enrollIdentities"] = func(m *model.Model) model.Action {
