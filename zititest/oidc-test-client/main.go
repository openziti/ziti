...
type config struct {
	...
	// clientId is this driver's own component ID. In sdk-direct mode it is also
	// the ClientId stamped on emitted events, since there's no rotation over
	// destinations. In both modes it identifies the driver on drop reports.
	clientId string
...
const (
	// resultsWriteTimeout bounds a single write to the results service. Edge conns
	// default to an effectively infinite timeout, so without this a half-open
	// results circuit would park the sender forever.
	resultsWriteTimeout = 10 * time.Second

	// resultsQueueDepth is how many events may be pending delivery. The traffic
	// loops emit a few events per second, so this absorbs a multi-minute results
	// outage before anything is dropped.
	resultsQueueDepth = 1024
)

// reporter forwards JSON events to a Ziti service. Events are queued and written
// by a single background goroutine, so a stalled or broken results circuit cannot
// block the traffic loops producing them. Events dropped because the queue filled
// are counted and reported to the collector as an error event once delivery
// recovers, so a reporting outage fails validation instead of looking identical to
// a clean run.
type reporter struct {
	...
	// closeStartNotify is closed to ask the sender to stop; closeCompleteNotify is
	// closed by the sender once it has drained and released its connection.
	closeStartNotify    chan struct{}
	closeCompleteNotify chan struct{}

	// dialResults opens a connection to the results service. It is a field so
	// tests can supply a connection without standing up a Ziti context.
	dialResults func() (net.Conn, error)

	// writeTimeout bounds a single write; defaults to resultsWriteTimeout.
	writeTimeout time.Duration

	// conn is owned exclusively by run; no other goroutine may touch it.
	conn net.Conn
}

// newReporter starts a reporter delivering to service. clientId identifies this
// driver and is stamped on drop reports. The caller must call close.
func newReporter(identityFile, service, clientId string) (*reporter, error) {
	...
	r := &reporter{
		ctx:                 ctx,
		clientId:            clientId,
		events:              make(chan trafficEvent, resultsQueueDepth),
		closeStartNotify:    make(chan struct{}),
		closeCompleteNotify: make(chan struct{}),
		writeTimeout:        resultsWriteTimeout,
	}
	r.dialResults = func() (net.Conn, error) { return ctx.Dial(service) }
...
// send queues evt for delivery. It never blocks: when the queue is full the event
// is dropped and counted, and that count reaches the collector once delivery
// recovers.
func (r *reporter) send(evt trafficEvent) {
	select {
	case r.events <- evt:
	default:
		// Log the first drop and then sparsely; a full queue means many in a row.
		if n := r.dropped.Add(1); n == 1 || n%100 == 0 {
			pfxlog.Logger().Warnf("results queue full, %d events dropped so far", n)
		}
	}
}

func (r *reporter) run() {
	defer close(r.closeCompleteNotify)
	defer r.closeConn()

	for {
		select {
		case <-r.closeStartNotify:
			r.drain()
			return
		case evt := <-r.events:
			r.deliver(evt)
...
// drain flushes whatever is already queued at shutdown, best effort.
...
// deliver writes evt, preceded by a drop report if events have been lost since the
// last one. Only the number actually reported is subtracted, so drops recorded by
// the traffic loops while the report is in flight survive, and a report that fails
// to send leaves the count intact for the next attempt.
func (r *reporter) deliver(evt trafficEvent) {
	if n := r.dropped.Load(); n > 0 && r.write(r.dropEvent(n)) {
		r.dropped.Add(-n)
	}
	r.write(evt)
}

// write sends one event, dialing the results service if there is no live
// connection. It reports whether the event reached the wire; on failure the
// connection is discarded so the next attempt redials.
func (r *reporter) write(evt trafficEvent) bool {
	log := pfxlog.Logger()

	data, err := json.Marshal(evt)
	if err != nil {
		log.WithError(err).Error("failed to marshal results event")
		return false
	}
	data = append(data, '\n')

	if r.conn == nil {
		conn, err := r.dialResults()
		if err != nil {
			log.WithError(err).Warn("failed to dial results service")
			return false
		}
		r.conn = conn
	}
	...
	if err = r.conn.SetWriteDeadline(time.Now().Add(r.writeTimeout)); err != nil {
		log.WithError(err).Warn("failed to set results write deadline, reconnecting")
		r.closeConn()
		return false
	}

	if _, err = r.conn.Write(data); err != nil {
		log.WithError(err).Warn("failed to write to results service, reconnecting")
		r.closeConn()
		return false
	}
...
// dropEvent builds the error event telling the collector that n events were lost.
// It carries this driver's own client id rather than a dial target's, so a
// reporting outage is not misattributed to whichever target was being dialed.
func (r *reporter) dropEvent(n int64) trafficEvent {
	return trafficEvent{
		Type:      "results-drop",
		Status:    "error",
		Error:     fmt.Sprintf("dropped %d results events; results service was unreachable", n),
		ClientId:  r.clientId,
		Timestamp: now(),
	}
}
...
// stop shuts down the sender, flushing whatever is already queued. It returns
// once the sender has stopped, so callers may then tear down what it was using.
func (r *reporter) stop() {
	close(r.closeStartNotify)
	<-r.closeCompleteNotify
}

// close stops the sender, flushing what it can, and releases the Ziti context.
...
func main() {
	...
	case "sdk-direct":
		...
		dialer = d
		closer = c
		destinations = []destination{{ClientId: cfg.clientId}}
	case "proxy":
	...
	if resultsIdentity != "" && cfg.resultsService != "" {
		var err error
		results, err = newReporter(resultsIdentity, cfg.resultsService, cfg.clientId)
		if err != nil {
	...
	emit := func(evt trafficEvent) {
		// stdout, which the launcher discards; the collector is the record of
		// truth and reporting failures surface through the reporter's drop count.
...
func parseFlags() config {
	...
	flag.StringVar(&cfg.clientId, "client-id", "", "This driver's component ID, used for drop reports and sdk-direct events")
