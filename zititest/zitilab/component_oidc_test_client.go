...
func (self *OidcTestClientType) Start(run model.Run, c *model.Component) error {
	...
	mode := self.modeString()
	// --client-id identifies the driver itself in both modes: sdk-direct stamps it
	// on emitted events, and either mode uses it to attribute dropped results.
	args := fmt.Sprintf("--mode %s --services %s --client-id %s", mode, self.Services, c.Id)
