...
func ValidateOidcNewSessions(collector *zitilibOps.OidcEventCollector, expectedIds map[string]bool, since time.Time, timeout time.Duration) error {
	return waitForIdentityEvents(collector, expectedIds, since, timeout,
		"created new OIDC sessions", collector.CreatedIdentitiesSince)
}

// ValidateOidcRefreshed waits until every identity in expectedIds has a JWT
// "refreshed" or "exchanged" event since the given timestamp. Use this after a
// partition heal where refresh tokens remained valid, to confirm clients
// recovered via the refresh-token path rather than a full re-authentication.
func ValidateOidcRefreshed(collector *zitilibOps.OidcEventCollector, expectedIds map[string]bool, since time.Time, timeout time.Duration) error {
	return waitForIdentityEvents(collector, expectedIds, since, timeout,
		"refreshed OIDC tokens", collector.RefreshedIdentitiesSince)
}

// waitForIdentityEvents waits until every identity in expectedIds appears in the
// set returned by getIds for the given timestamp. desc names the event kind being
// waited on, and is used in progress logging and the timeout error.
func waitForIdentityEvents(collector *zitilibOps.OidcEventCollector, expectedIds map[string]bool, since time.Time,
	timeout time.Duration, desc string, getIds func(time.Time) map[string]bool) error {
	log := tui.ValidationLogger()
	deadline := time.Now().Add(timeout)
	expectedCount := len(expectedIds)
	...
	for {
		missing := missingIds(expectedIds, getIds(since))

		if len(missing) == 0 {
			log.Infof("all %d expected identities %s since %s", expectedCount, desc, sinceStr)
			return nil
		}
		...
		if !time.Now().Before(deadline) {
			...
			return fmt.Errorf("%d of %d identities have not %s since %s. sample: %v (total events: %d)",
				len(missing), expectedCount, desc, sinceStr, sample, collector.TotalEventCount())
		}

		if time.Since(lastLog) > 15*time.Second {
			log.Infof("%d / %d identities %s since %s (total events: %d), waiting...",
				expectedCount-len(missing), expectedCount, desc, sinceStr, collector.TotalEventCount())
			lastLog = time.Now()
		}
		time.Sleep(5 * time.Second)
	}
...
func validateConnectionStatusesForCtrl(run model.Run, c *model.Component, deadline time.Time) error {
	...
	for {
		...
		count, err := checkConnectionStatuses(c.Id, clients, true)
		if err == nil {
			return nil
		}

		if time.Now().After(deadline) {
			// Dump full per-router error details before returning so the failure
			// report has enough context to diagnose.
			_, _ = checkConnectionStatuses(c.Id, clients, false)
			return err
		}
...
// checkConnectionStatuses asks the controller to validate identity connection
// statuses across all routers. When summary is true, logs only the count of
// invalid entries and the list of routers reporting errors; when false, logs
// each individual error with full detail.
func checkConnectionStatuses(ctrlId string, clients *zitirest.Clients, summary bool) (int, error) {
	...
	for expected > 0 {
		...
		case routerDetail := <-eventNotify:
			if len(routerDetail.Errors) > 0 {
				if summary {
					invalidRouters = append(invalidRouters,
						fmt.Sprintf("%s(%d)", routerDetail.ComponentName, len(routerDetail.Errors)))
				} else {
				...
				invalid += len(routerDetail.Errors)
			}
			expected--
		}
	}
	...
	if summary {
		logger.Infof("invalid identity connection statuses: %d across %d routers: %v",
			invalid, len(invalidRouters), invalidRouters)
	}
