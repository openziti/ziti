...
func validateServiceTerminatorsForCtrl(run model.Run, c *model.Component, deadline time.Time, expectations []ServiceTerminatorExpectation, validationType TerminatorValidationType) error {
	...
	lastCountErr := errors.New("terminator counts were never successfully queried")
	...
	for time.Now().Before(deadline) {
		...
		for _, exp := range expectations {
			...
			if count != exp.ExpectedCount {
				...
				lastCountErr = fmt.Errorf("service %s has %d terminators, expected %d", exp.ServiceName, count, exp.ExpectedCount)
				allSatisfied = false
				break
			}
		}
	...
	if !countsSatisfied {
		return fmt.Errorf("timed out after %v waiting for expected terminator counts on ctrl %s: %w", time.Since(start), c.Id, lastCountErr)
	}
