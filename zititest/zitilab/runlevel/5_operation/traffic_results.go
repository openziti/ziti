...
func (self *TrafficResultsCollector) handleConnection(conn net.Conn) {
	...
	// Without this a read error (a client restart resetting its results circuit, or
	// a line over the 64KB buffer) would end collection for this client silently.
	if err := scanner.Err(); err != nil {
		log.WithError(err).Info("traffic results connection read error")
	}
}
