...
func (self *ZitiProxCType) StageFiles(r model.Run, c *model.Component) error {
	...
	// LocalPath is the ziti-prox-c build, not a ziti build, so it must not be reused as the ziti
	// source: with an empty ZitiVersion that would stage the prox executable as "ziti", and the
	// shared staged binary would then fail when invoked as "ziti router enroll". An empty source
	// resolves ziti from ZITI_PATH/PATH instead.
	return stageziti.StageZitiOnce(r, c, self.ZitiVersion, "")
}
...
func (self *ZitiProxCType) getProcessFilter(c *model.Component) func(string) bool {
	return func(s string) bool {
		// Match the version-agnostic base binary name, not getBinaryName() (which includes the
		// version): the component's config file already identifies its process uniquely, and matching
		// the versioned name would miss a running instance after a version change, leaving the
		// old-version process alive on stop/restart.
		return strings.Contains(s, "ziti-prox-c") &&
