...
func (sim *Sim) InitConnectors() error {
	...
	for name, connector := range sim.scenario.ConnectorConfigs {
		if connector.SdkOptions != nil {
			...
			return fmt.Errorf("connector '%s' has no conifguration for sdk or transport", name)
		}
	}
	return nil
}
...
func (sim *Sim) DumpMetrics() {
	sim.metrics.AcceptVisitor(metricsEmitter{})
}
...
func (m metricsEmitter) VisitMeter(name string, metric metrics.Meter) {
	...
	if strings.Contains(name, "bytes") {
		fmt.Printf("%s.avg: %v\n", name, outputz.FormatBytes(uint64(metric.Rate1())))
	} else {
		fmt.Printf("%s.rate_1m: %v\n", name, metric.Rate1())
	}
}
...
func (m metricsEmitter) VisitHistogram(name string, metric metrics.Histogram) {
	fmt.Printf("%s.count: %v\n", name, metric.Count())
	fmt.Printf("%s.avg: %v\n", name, metric.Mean())
	fmt.Printf("%s.p95: %v\n", name, metric.Percentile(0.95))
}

func (m metricsEmitter) VisitTimer(name string, metric metrics.Timer) {
	fmt.Printf("%s.count: %v\n", name, metric.Count())
	fmt.Printf("%s.rate_1m: %v\n", name, metric.Rate1())
	fmt.Printf("%s.avg: %v\n", name, time.Duration(int64(metric.Mean())).String())
	fmt.Printf("%s.p95: %v\n", name, time.Duration(int64(metric.Percentile(0.95))))
}
