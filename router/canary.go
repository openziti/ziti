...
func newCanaryEmitter(ctrls env.NetworkControllers, epoch []byte,
	...
	// Seeded from the epoch, not zero: the sequence is the version a controller stores the canary under, and it
	// refuses anything not newer, so a restarted router has to start above whatever its predecessor reached.
	// The epoch advances by the whole elapsed wall time while the sequence only ticks, so the new epoch already
	// exceeds the sequence the previous lifetime reached.
	result.seq.Store(idgen.EpochMillis(epoch))
