...
func (self *testSetup) runTestVariation(env *testEnv, t *testing.T) {
	...
	if env.closeUp || env.closeDown {
		start := time.Now()
		for !(env.clientXg.IsClosed() && env.hostXg.IsClosed()) {
			time.Sleep(100 * time.Millisecond)
			if time.Since(start) > 2*time.Second {
				debugz.DumpStack()
				req.True(env.clientXg.IsClosed())
				req.True(env.hostXg.IsClosed())
				return
			}
		}
	}
...
func (self *testSetup) NewSdkConn(originator xgress.Originator) (net.Conn, xgress.Connection) {
	sdkConn := NewBufferPipe(originator.String())
	listener := channel.NewExistingConnListener(&identity.TokenId{Token: "test"}, sdkConn.Right(), nil)
	dialer := channel.NewExistingConnDialer(&identity.TokenId{Token: "test"}, sdkConn.Left(), nil)

	listener := xgress_edge.NewAcceptor()
}
