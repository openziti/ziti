...
type Registry struct {
	...
	// wgMu serializes spawnReconcile's closed-check + wg.Add against Close's
	// transition + wg.Wait, so a reconcile can never call wg.Add concurrently
	// with (or after) the wg.Wait in Close. closed stays atomic for the
	// best-effort early-out in reconcileAsync, which doesn't hold wgMu.
	wgMu   sync.Mutex
	wg     sync.WaitGroup
	closed atomic.Bool
}
