...
func LoadManagedConfigFromMap(cfgmap map[interface{}]interface{}, options *ManagedConfigOptions) error {
	...
	for i, item := range allowList {
		// This is a security boundary, so reject non-string entries rather than
		// coercing values like `true` or `123` into config type names.
		s, ok := item.(string)
		if !ok {
			return fmt.Errorf("[managedConfig/allow][%d] must be a string, got %T", i, item)
		}
		options.Allow = append(options.Allow, s)
	}
	return nil
}
