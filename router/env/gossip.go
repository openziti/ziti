...
type LinkGossipNotifier interface {
	...
	// InspectGossipLinks returns the router's own published link gossip entries
	// (the set advertised to controllers via anti-entropy). Diffing this against
	// the link registry reveals registry<->gossip divergence.
	InspectGossipLinks() *inspect.RouterGossipLinksInspect
	// InspectGossipStore returns summary stats for the router's local gossip store.
	InspectGossipStore() *inspect.RouterGossipStoreInspect
}
