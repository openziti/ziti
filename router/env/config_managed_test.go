...
func Test_ManagedConfig_RejectsNonStringAllowEntry(t *testing.T) {
	req := require.New(t)
	// The allow-list is a security boundary; non-string YAML values must be
	// rejected rather than coerced into config type names.
	for _, bad := range []interface{}{true, 123} {
		opts := &ManagedConfigOptions{}
		err := LoadManagedConfigFromMap(map[interface{}]interface{}{
			"managedConfig": map[interface{}]interface{}{
				"allow": []interface{}{"router.link", bad},
			},
		}, opts)
		req.Error(err)
