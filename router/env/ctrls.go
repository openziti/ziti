...
type NetworkControllers interface {
	// Start is TEMPORARY DIAGNOSTIC CODE. See the implementation for what it does and when to remove it.
...
// Start begins reporting, until closeNotify is closed, on controllers the router has lost its connection to
// without recovering it. It has no effect if the check is disabled by configuration.
//
// TEMPORARY DIAGNOSTIC CODE - REMOVE BEFORE MERGING.
//
// This exists to prove that the control channel recovery paths do what they claim, after a run of fixes to
// them. It takes no corrective action, so nothing depends on it and removing it changes no behaviour. It
// has not yet run through a full soak, which is the evidence it was written to collect: if a 24h run over
// the full router fleet reports nothing, it has done its job and should go. If it does report something,
// that is a defect to fix at its source, and this still goes once that fix is in.
//
// To remove: delete Start, checkConnections, trackUnreachable and unusableRegistration here, the
// unusableSince field on networkControllers, Start from the NetworkControllers interface and its mocks, the
// call in router/router.go, and ConnectionCheckInterval/ConnectionCheckGracePeriod from router/env/config.go.
