...
// AddReceiveHandlerF / AddTypedReceiveHandler install a timing-wrapped
// version of the handler. Other Binding methods are forwarded unchanged.
...
func (b *slowHandlerBinding) AddTypedReceiveHandler(h channel.TypedReceiveHandler) {
	b.Binding.AddTypedReceiveHandler(&timedTypedReceiveHandler{inner: h})
}
...
type timedTypedReceiveHandler struct {
	inner channel.TypedReceiveHandler
}

func (t *timedTypedReceiveHandler) ContentType() int32 {
	return t.inner.ContentType()
}

func (t *timedTypedReceiveHandler) HandleReceive(m *channel.Message, ch channel.Channel) {
	start := time.Now()
	t.inner.HandleReceive(m, ch)
	recordHandlerDuration(t.inner.ContentType(), ch, time.Since(start))
}
