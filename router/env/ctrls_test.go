...
// awaitCtrlEvents collects n controller events, failing if they do not arrive in time. Collection happens on
// the calling goroutine, since testify runs Eventually and Never conditions on their own goroutine and
// accumulating into a slice the test body also reads would race.
...
func TestEverConnected_StaysTrueAcrossLosingEveryController(t *testing.T) {
	...
	// Lose it, leaving no controllers at all, which is the state a check on the instant count trips on.
	nc.handleChannelClose("ctrl1", ctrl, 0)
...
func TestNotifyOfConnectivityChange_ReconnectReportsBothHalves(t *testing.T) {
	...
	// Listeners are notified on a goroutine per event, so the two can arrive in either order.
...
func TestNotifyOfConnectivityChange_ReportsEachEdgeOnce(t *testing.T) {
	...
	// Waiting for each edge before triggering the next fixes their order, unlike the back-to-back case.
	require.Len(t, events, 2, "each edge must be reported once, got %v", events)
	require.Equal(t, ControllerDisconnected, events[0].Type)
