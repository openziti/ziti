...
func (self *checkStaleLinksHandler) computeReports(wireMode ctrl_pb.StaleLinkMatchMode) []*ctrl_pb.LinkStaleReport {
	...
	destListeners, haveSnapshot := registry.GetDestinationListeners()
	...
	for xl := range registry.Iter() {
		...
		if xl.IsDialed() {
			if !haveSnapshot {
				// Without a reliable destination listener snapshot we can't
				// judge the dialer side. Omit the report so the controller
				// sees a partial result (no GC) rather than a false "stale".
				continue
			}
