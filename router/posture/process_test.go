...
// reportedProcessResponse builds a single-entry process list response for the required path with
// the given reported hash and signer fingerprints.
func reportedProcessResponse(hash string, fingerprints []string) *edge_client_pb.PostureResponse {
...
func Test_ProcessCheck_ReportedValuesNormalizedOnIngest(t *testing.T) {
	...
	updated := instance.Apply(reportedProcessResponse(upperHash, []string{separatedPrint}), nil)
...
func Test_ProcessCheck_NormalizationDoesNotMutateResponse(t *testing.T) {
	instance := newInstance()
	response := reportedProcessResponse(upperHash, []string{separatedPrint})
...
func Test_ProcessCheck_RepeatedResponseNotSeenAsChange(t *testing.T) {
	instance := newInstance()
	require.True(t, instance.Apply(reportedProcessResponse(upperHash, []string{separatedPrint}), nil))
