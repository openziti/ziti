...
// reportedProcessEntry builds a running process entry at the required path with the given reported
// hash and signer fingerprints, for feeding through Instance.Apply.
func reportedProcessEntry(hash string, fingerprints []string) *edge_client_pb.PostureResponse_Process {
	return &edge_client_pb.PostureResponse_Process{
		Path:               testProcPath,
		IsRunning:          true,
		Hash:               hash,
		SignerFingerprints: fingerprints,
...
func Test_ProcessCheck_ReportedValuesNormalizedOnIngest(t *testing.T) {
	...
	updated := instance.Apply(processListResponse(reportedProcessEntry(upperHash, []string{separatedPrint})), nil)
...
func Test_ProcessCheck_NormalizationDoesNotMutateResponse(t *testing.T) {
	instance := newInstance()
	response := processListResponse(reportedProcessEntry(upperHash, []string{separatedPrint}))
...
func Test_ProcessCheck_RepeatedResponseNotSeenAsChange(t *testing.T) {
	instance := newInstance()
	require.True(t, instance.Apply(processListResponse(reportedProcessEntry(upperHash, []string{separatedPrint})), nil))
