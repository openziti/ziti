...
func (self *Acceptor) BindChannel(binding channel.Binding) error {
	...
	srcAddr := conn.ch.Underlay().GetRemoteAddr().String()
	dstAddr := conn.ch.Underlay().GetLocalAddr().String()
	self.connStateTracker.markConnected(identityId, srcAddr, dstAddr)

	binding.AddCloseHandler(channel.CloseHandlerF(func(ch channel.Channel) {
		self.connectionCount.Add(-1)
		self.disconnectMeter.Mark(1)
		self.connStateTracker.markDisconnected(identityId)
	}))
...
func NewAcceptor(listener *listener, uListener channel.UnderlayListener, options *channel.Options) *Acceptor {
	...
	result := &Acceptor{
		...
		connStateTracker:    newConnectionTracker(listener.factory.env),
