...
func (e *minimalLinkEnv) GetMetricsRegistry() servermetrics.UsageRegistry {
	panic("not used in close tests")
}
func (e *minimalLinkEnv) GetXLinkRegistry() xlink.Registry { panic("not used in close tests") }
func (e *minimalLinkEnv) GetNetworkControllers() env.NetworkControllers {
	panic("not used in close tests")
}
...
func Test_Listener_Close_RejectsInFlightAccept(t *testing.T) {
	req := require.New(t)
	l := newCloseTestListener(newMinimalLinkEnv())

	req.NoError(l.Close())

	// An accept that was already in flight when Close drained pendingLinks must
	// not re-populate the map: the cleanup goroutine has exited, so anything
	// added now would be held until the process exits.
	complete, link, err := l.getOrCreateSplitLink("conn-1", &linkMetadata{linkId: "l1"}, nil, PayloadChannel)
	req.ErrorIs(err, errListenerClosed)
	req.False(complete)
	req.Nil(link)
	req.Empty(l.pendingLinks, "closed listener must not retain partial links")
}

func Test_Listener_Close_WaitsForInFlightAccept(t *testing.T) {
	req := require.New(t)
	l := newCloseTestListener(newMinimalLinkEnv())

	// An accept that started before the close must finish before Close returns,
	// so the caller can rely on no further links being registered once a
	// managed-config change has removed the listener.
	req.True(l.beginAccept())
	...
	select {
	case <-closeReturned:
		t.Fatal("Close returned while an accept was still in flight")
	...
	case <-closeReturned:
	case <-time.After(2 * time.Second):
		t.Fatal("Close did not return after the in-flight accept finished")
	}
}

func Test_Listener_Close_PendingLinksDrainedAfterInFlightAccept(t *testing.T) {
	req := require.New(t)
	l := newCloseTestListener(newMinimalLinkEnv())
	...
	// Let Close get as far as waiting on the in-flight accept, then have that
	// accept insert a partial link the way a real one would. Close must sweep
	// it rather than return with the entry still held.
	time.Sleep(50 * time.Millisecond)
	l.lock.Lock()
	l.pendingLinks["conn-late"] = &pendingLink{link: &splitImpl{}, eventTime: time.Now()}
	l.lock.Unlock()
	...
	l.lock.Lock()
	defer l.lock.Unlock()
	req.Empty(l.pendingLinks, "partial link inserted by an in-flight accept must be swept by Close")
}

func Test_Listener_Close_RefusesNewAcceptSlots(t *testing.T) {
	req := require.New(t)
	l := newCloseTestListener(newMinimalLinkEnv())
	...
	req.NoError(l.Close())
	req.False(l.beginAccept(), "a closed listener must not hand out accept slots")
}

func Test_Listener_Close_RejectsNewUnderlays(t *testing.T) {
	req := require.New(t)
	l := newCloseTestListener(newMinimalLinkEnv())

	req.NoError(l.Close())

	req.ErrorIs(l.handleUngroupedNewUnderlay(nil), errListenerClosed)

	ch, err := l.handleGroupedUnderlay(nil, func() {})
	req.ErrorIs(err, errListenerClosed)
	req.Nil(ch)
}
