...
// so whatever the base does on removal is all that happens.
...
// TestConnIteration_DialSideAdvancesOnceOnRemove: the base handler advances the iteration on removal, so
// the dial side must not advance it again. Doing so would move the iteration twice for one change and pull
// the two ends of a link further out of step.
