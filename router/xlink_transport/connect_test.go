...
func Test_ConnectionHandler_RejectsUntrustedDialer(t *testing.T) {
	...
	req.Error(handler.HandleConnection(nil, nil), "no certificates must be rejected")
	req.Error(handler.HandleConnection(nil, []*x509.Certificate{forged.cert}),
		"self-signed leaf that does not chain to the CA must be rejected")
	req.Error(handler.HandleConnection(nil, []*x509.Certificate{forged.cert, legit.cert}),
