...
func (g *gossipClient) NotifyLinkFault(linkId string, iteration uint32) error {
	...
	// Mutate local tracking only after a successful send. Decrementing before
	// the send drifts the count when sendDelta fails: sendLinkFaults retries
	// NotifyLinkFault, the retry sees Has(key)==false and skips the decrement,
	// leaving the canary advertising a count lower than the controller's
	// view and triggering needless digest exchanges.
