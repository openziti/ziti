...
const (
	...
	// gossipDigestResponseSendTimeout bounds the digest response send. Anti-entropy repeats, so giving up
	// beats waiting on a control channel that is not draining.
	gossipDigestResponseSendTimeout = 5 * time.Second
)
...
type gossipStore struct {
	...
	cached         atomic.Pointer[hashCount]
}
...
//
// generation is the store's mutation count at the time the pass began, which is what makes the cache safe to
// publish without holding off mutations. A pass that started before a mutation stores a value stamped with the
// older generation, so the next reader sees it does not describe the current set and recomputes. Clearing the
// cache on mutation instead would let such a pass overwrite the clear, leaving a wrong hash cached with
// nothing left to invalidate it until the next mutation, which for a router with stable links is indefinitely.
...
// invalidateHash records that currentEntries changed, so a cached hash+count no longer describes it, and
// stamps the change time. Called on every currentEntries mutation.
...
func (s *gossipStore) getCached() *hashCount {
	// Read before the pass, not after: a mutation during the pass then leaves the stored value stamped with a
	// generation the store has moved past, so it is recomputed rather than trusted.
	generation := s.generation.Load()
	if hc := s.cached.Load(); hc != nil && hc.generation == generation {
		return hc
	}
	...
	hc := &hashCount{hash: h.Sum64(), count: int64(len(kvs)), generation: generation}
	s.cached.Store(hc)
	return hc
}
...
type gossipClient struct {
	...
	// pool supplies the receive pool digest handling runs on. Late-bound, since the pool is built after the
	// gossip client. Nil until set, in which case digests are handled inline.
	pool atomic.Value // holds goroutines.Pool
}

// setPool supplies the receive pool digest handling runs on. Called once during startup, once it exists.
func (g *gossipClient) setPool(p goroutines.Pool) {
	if p != nil {
		g.pool.Store(p)
	}
}

func (g *gossipClient) getPool() goroutines.Pool {
	if v := g.pool.Load(); v != nil {
		if p, ok := v.(goroutines.Pool); ok {
			return p
		}
	}
	return nil
...
// allocated version.
//
// No expiry is set: how long a tombstone is retained is the controller's policy, not the router's, so the
// first controller to receive it stamps the deadline and replicates that to its peers.
...
// sendDelta publishes entries to one controller, reporting whether they were sent.
//
// A failed send is reported as one even when the channel has since closed. Every caller decides what to do
// with its own bookkeeping from this answer: whether an entry stays in the advertised set, whether a link
// counts as notified, whether a fault stays pending, and how high a version the router claims to have sent.
// Returning success for a closed channel left all of that saying the state had been published when the
// controller never saw it, with nothing local left to retry and only a later digest exchange to notice.
//
// A send that failed after the bytes reached the wire is possible, and retrying it is harmless: entries carry
// versions, so applying one twice is a no-op, and a tombstone is idempotent.
...
// HandleDigest receives a controller's digest and answers with what the controller is missing.
//
// Only the unmarshal happens on the channel's receive goroutine. The comparison walks every entry the
// controller reported and every entry this router holds, which in a large mesh is one per link, and it
// finishes by sending a response. Doing that on the receive goroutine stops the router reading anything else
// from that control channel until it completes, which was measured stalling control channels for a
// half second at the median and up to four seconds, and is what the controller sees as send back-pressure.
//
// Dropping a digest exchange when the pool is full is safe: it is anti-entropy, so the next round repairs
...
func (g *gossipClient) HandleDigest(msg *channel.Message, ch channel.Channel) {
	...
	if err := pool.QueueOrError(func() { g.handleDigest(digest, ch) }); err != nil {
		pfxlog.Logger().WithField("storeType", digest.StoreType).
			Warn("pool full, dropping gossip digest exchange, anti-entropy will repair")
	}
}

func (g *gossipClient) handleDigest(digest *gossip_pb.GossipDigest, ch channel.Channel) {
	s := g.stores[digest.StoreType]
	if s == nil {
		return
	}

	// Build map of what the controller has, and advance our clock past the highest version so anything
	// we stamp below outranks what the controller already holds. This is critical after a restart, when
	// the Lamport clock resets to 0 and the controller still holds versions from the previous
	// incarnation. Both the tombstone loop and the re-stamp below depend on this running first.
	...
	s.source.iterateAdvertised(func(key string, value []byte) {
		...
		if stored, ok := s.currentEntries.Get(key); ok && !stored.Tombstone {
			...
			// The controller holds a higher version than we do for a key we own and still advertise: a
			// value or tombstone left by a previous incarnation, which our restarted clock cannot climb
			// past by publishing. Keeping our version would send nothing, and every later digest would
			// reach the same conclusion, so the divergence would never be repaired by the exchange whose
			// job that is. Re-stamp above the controller instead and send the live value.
			//
			// Safe only because ownership here is one way: the router is the sole writer of the keys it
			// advertises and never applies an inbound value for them, so raising our own version cannot
			// shadow an update we should have taken. An ingest path for owned keys would break that.
	...
	// Bounded rather than a bare Send: an unbounded send blocks its goroutine until the peer drains, which
	// pins a pool worker on exactly the back-pressure this exchange is meant to relieve.
	if err := respMsg.WithTimeout(gossipDigestResponseSendTimeout).Send(ch); err != nil {
		if !ch.IsClosed() {
