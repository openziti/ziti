...
type Registry interface {
	...
	// GetDestinationListeners returns the listener set most recently
	// advertised by each healthy destination router, keyed by router id,
	// for use by stale-link checks. The bool is false when no snapshot could
	// be produced (e.g. the registry event loop didn't respond in time).
	//
	// An absent destination means its listeners are unknown, never that it
	// has none; callers must not treat either absence as evidence.
...
type Xlink interface {
	...
	// the link so its intervals can be adjusted later. A split link has one per
	// underlying channel.
