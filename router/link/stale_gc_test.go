...
type gcStubRegistry struct {
	...
	snapshotUnavailable bool // when true, GetDestinationListeners reports failure
}
...
func (r *gcStubRegistry) GetDestinationListeners() (map[string][]*ctrl_pb.Listener, bool) {
	if r.snapshotUnavailable {
		return nil, false
	}
	return r.destListeners, true
}
...
func Test_RunStaleLinkGc_IndeterminateSnapshot_KeepsPeerDependentLinks(t *testing.T) {
	// Only the peer's listener set could make this stale, and it's unavailable.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	env := &gcStubEnv{
	...
	closed := RunStaleLinkGc(env, GcModeOrphaned)
	require.Empty(t, closed, "an indeterminate snapshot must not close peer-dependent links")
	require.False(t, link.IsClosed())
}

func Test_RunStaleLinkGc_IndeterminateSnapshot_StillClosesLocallyStaleLinks(t *testing.T) {
	// A missing dialer binding is decidable without the peer.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	env := &gcStubEnv{
		registry: &gcStubRegistry{
			links:               []xlink.Xlink{link},
		...
		dialers: []xlink.Dialer{&stubXlinkDialer{binding: "ws"}}, // binding gone -> stale
	}
	closed := RunStaleLinkGc(env, GcModeOrphaned)
	require.Len(t, closed, 1)
	require.True(t, link.IsClosed())
...
func Test_RunStaleLinkGc_UnhealthyPeer_KeepsLink(t *testing.T) {
	// An unhealthy peer drops out of the snapshot; that must not read as
	// "no listeners" and close a live link.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	env := &gcStubEnv{
		registry: &gcStubRegistry{
			links: []xlink.Xlink{link},
			// destA is absent; some other peer is known and healthy.
			destListeners: map[string][]*ctrl_pb.Listener{
				"destB": {{Protocol: "tls", LocalBinding: "lb1", Groups: []string{"g1"}}},
			},
		},
		dialers: []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}},
	}

	for _, mode := range []GcMode{GcModeOrphaned, GcModeChanged} {
		closed := RunStaleLinkGc(env, mode)
		require.Empty(t, closed, "mode %v must not close a link to an unknown destination", mode)
		require.False(t, link.IsClosed(), "mode %v", mode)
	}
}
...
func Test_RunStaleLinkGc_NoLinkSurfaceClosesEverything(t *testing.T) {
	// The state after the link config is removed: no listeners, no dialers. No
	// remaining link can be re-established, so under orphaned or changed they
	// are all collected. This is what makes sweeping a removal worthwhile; under
	// preserve they are still left alone.
	dialed := dialedXlink("transport", "tls", "destA", "lb1")
	accepted := &stubXlink{
		dialAddress:  "tls:1.2.3.4:6000",
		linkProtocol: "tls",
		linkKey:      xlink.LinkKey{Protocol: "tls", ListenerBinding: "lb1"},
	...
	newEnv := func() *gcStubEnv {
		return &gcStubEnv{
			registry: &gcStubRegistry{
				links: []xlink.Xlink{dialed, accepted},
				destListeners: map[string][]*ctrl_pb.Listener{
					"destA": {{Protocol: "tls", LocalBinding: "lb1", Groups: []string{"g1"}}},
				},
			},
	...
	closed := RunStaleLinkGc(newEnv(), GcModePreserve)
	require.Empty(t, closed, "preserve still collects nothing")
	require.False(t, dialed.IsClosed())
	require.False(t, accepted.IsClosed())

	closed = RunStaleLinkGc(newEnv(), GcModeOrphaned)
	require.Len(t, closed, 2, "both sides are unsupportable once the surface is gone")
	require.True(t, dialed.IsClosed())
	require.True(t, accepted.IsClosed())
}
