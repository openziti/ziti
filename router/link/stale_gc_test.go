...
func Test_RunStaleLinkGc_IndeterminateSnapshot_SkipsSweep(t *testing.T) {
	// A dialed link that would be judged stale (wrong dialer binding), but the
	// destination listener snapshot is unavailable. GC must not close it.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	env := &gcStubEnv{
	...
	closed := RunStaleLinkGc(env, GcModeOrphaned)
	require.Empty(t, closed, "an indeterminate snapshot must not close any links")
	require.False(t, link.IsClosed())
}
