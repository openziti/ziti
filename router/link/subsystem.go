...
type ConfigurationChange struct {
	...
	DialersChanged bool
	// GcModeChanged is true when the gcMode field transitioned.
	GcModeChanged bool
	// GcMode is the auto-GC policy this change should be swept under. Carried
	// on the change rather than read back from the subsystem so a removal is
	// still swept: Remove clears the config before notifying, leaving nothing
...
// effectiveGcMode resolves the policy a change is swept under.
//
// An apply uses the mode its config carries, and an unparseable value falls back
// to preserve so a bad mode never widens what gets closed.
//
// A removal always sweeps, whatever the removed config said. gcMode governs
// links the router's own configuration made stale, and after a removal there is
// no configuration: Remove closes every listener and drops every dialer, so the
// links that remain can carry traffic until they break and can never be
// re-established. Orphaned is enough to collect them, since with nothing left to
// dial or listen on they are stale under even the milder criterion.
func effectiveGcMode(next *Config) GcMode {
	if next == nil {
		return GcModeOrphaned
	}
	mode, err := ParseGcMode(getGcMode(next))
	if err != nil {
		pfxlog.Logger().WithError(err).Warn("auto-GC: ignoring invalid gcMode, treating as preserve")
		return GcModePreserve
	}
	return mode
}
