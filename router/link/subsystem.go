...
func (self *Subsystem) Apply(version int, data string) error {
	...
	// rollback, where a failed update is undone by re-applying the current config
	// verbatim. Comparing the raw config data covers every field the config
	// carries, so a change to any of them still applies. Checked before build so
	// a no-op rollback doesn't construct (and then leak) replacement listeners
	// that reserve resources in CreateListener.
