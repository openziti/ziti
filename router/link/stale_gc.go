...
//
// Only a positive stale verdict closes anything; Unknown is left alone.
//
...
func RunStaleLinkGc(env XlinkEnv, mode GcMode) []string {
	...
	if !ok {
		// Still worth sweeping: a nil snapshot only makes peer-dependent checks
		// Unknown, leaving the local-only ones decisive.
		pfxlog.Logger().Warn("auto-GC: no destination listener snapshot, limiting sweep to locally-decidable links")
	}
	...
	for xl := range registry.Iter() {
		...
		verdict, reason, side := evaluateStaleness(xl, dialers, listeners, destListeners, stalenessMode)
		if verdict == StaleVerdictNotStale {
			continue
		}
		log := pfxlog.Logger().
		...
		if verdict == StaleVerdictUnknown {
			log.Debug("auto-GC: leaving link alone, side can't judge staleness")
			continue
		}
...
func evaluateStaleness(
	...
	mode StalenessMode,
) (StaleVerdict, string, string) {
	if xl.IsDialed() {
		verdict, reason := CheckDialerSide(xl, dialers, destListeners, mode)
		return verdict, reason, "dialer"
	}
	verdict, reason := CheckListenerSide(xl, listeners, mode)
	return verdict, reason, "listener"
}
