...
func (*stubXlink) SetHeartbeatControl(channel.HeartbeatControl, xlink.HeartbeatSettings) {}
func (*stubXlink) UpdateHeartbeat(xlink.HeartbeatSettings)                               {}
func (*stubXlink) HeartbeatSettings() xlink.HeartbeatSettings {
...
func Test_CheckDialerSide_Orphaned_FullMatch(t *testing.T) {
	...
	verdict, reason := CheckDialerSide(link, dialers, compatibleDest(), StalenessModeOrphaned)
	require.Equal(t, StaleVerdictNotStale, verdict)
...
func Test_CheckDialerSide_Orphaned_BindingRemoved(t *testing.T) {
	...
	verdict, reason := CheckDialerSide(link, dialers, compatibleDest(), StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
	require.Contains(t, reason, "transport")
}

func Test_CheckDialerSide_Orphaned_NoDialersAtAll(t *testing.T) {
	link := dialedXlink("transport", "tls", "destA", "lb1")
	verdict, _ := CheckDialerSide(link, nil, compatibleDest(), StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
}

func Test_CheckDialerSide_UnknownDestinationIsNotStale(t *testing.T) {
	// Absent means "listeners unknown", not "peer has none".
	link := dialedXlink("transport", "tls", "destA", "lb1")
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}}

	for _, mode := range []StalenessMode{StalenessModeOrphaned, StalenessModeChanged} {
		verdict, reason := CheckDialerSide(link, dialers, nil, mode)
		require.Equal(t, StaleVerdictUnknown, verdict, "mode %v", mode)
		require.Contains(t, reason, "destA")

		// Another destination being known says nothing about this one.
		verdict, _ = CheckDialerSide(link, dialers, destSnapshot("destB", &ctrl_pb.Listener{
			Protocol:     "tls",
			LocalBinding: "lb1",
			Groups:       []string{"g1"},
		}), mode)
...
func Test_CheckDialerSide_MissingDialerIsStaleEvenWithoutSnapshot(t *testing.T) {
	// Local-only fact, so it decides even with the peer unknown. This is what
	// keeps GC working for a removed dialer.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "ws", groups: []string{"g1"}}}
	verdict, reason := CheckDialerSide(link, dialers, nil, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
	require.Contains(t, reason, "transport")
}
...
func Test_CheckDialerSide_Orphaned_GroupsNoLongerOverlap(t *testing.T) {
	...
	verdict, reason := CheckDialerSide(link, dialers, dest, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
...
func Test_CheckDialerSide_Orphaned_ProtocolMismatch(t *testing.T) {
	...
	verdict, _ := CheckDialerSide(link, dialers, dest, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
}
...
func Test_CheckDialerSide_Changed_FullMatch(t *testing.T) {
	link := dialedXlink("transport", "tls", "destA", "lb1")
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}}
	verdict, reason := CheckDialerSide(link, dialers, compatibleDest(), StalenessModeChanged)
	require.Equal(t, StaleVerdictNotStale, verdict)
	require.Empty(t, reason)
}
...
func Test_CheckDialerSide_Changed_ListenerBindingGone(t *testing.T) {
	...
	verdict, reason := CheckDialerSide(link, dialers, dest, StalenessModeChanged)
	require.Equal(t, StaleVerdictStale, verdict)
	require.Contains(t, reason, "lb1")

	verdictOrphaned, _ := CheckDialerSide(link, dialers, dest, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictNotStale, verdictOrphaned, "renamed listener with compatible config shouldn't trigger orphaned-stale")
}

func Test_CheckDialerSide_Changed_AdvertiseChanged(t *testing.T) {
	// The binding still exists, but it advertises somewhere else now, so a
	// re-dial wouldn't reach where this link actually went.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	link.dialAddress = "tls:peer:6000"
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}}
	dest := destSnapshot("destA", &ctrl_pb.Listener{
		Address:      "tls:peer:7000",
		Protocol:     "tls",
		LocalBinding: "lb1",
		Groups:       []string{"g1"},
	})
	...
	verdict, reason := CheckDialerSide(link, dialers, dest, StalenessModeChanged)
	require.Equal(t, StaleVerdictStale, verdict)
	require.Contains(t, reason, "tls:peer:6000")

	verdictOrphaned, _ := CheckDialerSide(link, dialers, dest, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictNotStale, verdictOrphaned,
		"orphaned tolerates a re-advertisement; a compatible listener still exists")
}

func Test_CheckDialerSide_Changed_AdvertiseUnchanged(t *testing.T) {
	link := dialedXlink("transport", "tls", "destA", "lb1")
	link.dialAddress = "tls:peer:6000"
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}}
	dest := destSnapshot("destA", &ctrl_pb.Listener{
		Address:      "tls:peer:6000",
		Protocol:     "tls",
		LocalBinding: "lb1",
		Groups:       []string{"g1"},
	})
...
func Test_CheckDialerSide_Changed_AddressMustBeOnTheMatchingBinding(t *testing.T) {
	// The dialed address is still advertised, but by a listener with a
	// different binding. A re-dial would key differently, so it doesn't count.
	link := dialedXlink("transport", "tls", "destA", "lb1")
	link.dialAddress = "tls:peer:6000"
	dialers := []xlink.Dialer{&stubXlinkDialer{binding: "transport", groups: []string{"g1"}}}
	dest := destSnapshot("destA",
		&ctrl_pb.Listener{Address: "tls:peer:7000", Protocol: "tls", LocalBinding: "lb1", Groups: []string{"g1"}},
		&ctrl_pb.Listener{Address: "tls:peer:6000", Protocol: "tls", LocalBinding: "lb2", Groups: []string{"g1"}},
	)
...
func Test_CheckListenerSide_Orphaned_ProtocolMatch(t *testing.T) {
	...
	verdict, _ := CheckListenerSide(link, listeners, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictNotStale, verdict)
}
...
func Test_CheckListenerSide_Orphaned_ProtocolGone(t *testing.T) {
	...
	verdict, reason := CheckListenerSide(link, listeners, StalenessModeOrphaned)
	require.Equal(t, StaleVerdictStale, verdict)
...
func Test_CheckListenerSide_Changed_FullMatch(t *testing.T) {
	...
	verdict, _ := CheckListenerSide(link, listeners, StalenessModeChanged)
	require.Equal(t, StaleVerdictNotStale, verdict)
}
...
func Test_CheckListenerSide_Changed_AdvertiseChanged(t *testing.T) {
	...
	verdict, reason := CheckListenerSide(link, listeners, StalenessModeChanged)
	require.Equal(t, StaleVerdictStale, verdict)
...
func Test_CheckListenerSide_Changed_NoListeners(t *testing.T) {
	...
	verdict, _ := CheckListenerSide(link, nil, StalenessModeChanged)
	require.Equal(t, StaleVerdictStale, verdict)
}
