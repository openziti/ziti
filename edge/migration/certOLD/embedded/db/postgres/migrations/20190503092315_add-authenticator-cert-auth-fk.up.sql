...
insert into ziti_edge.authenticators(identity_id, method)
    (SELECT
            identity_id,
            'cert' as type
     from ziti_edge.authenticator_certs);
...
update ziti_edge.authenticator_certs ac set authenticator_id = (select a.id from ziti_edge.authenticators a where a.identity_id = ac.identity_id) where 1=1;

-- make authenticator id not null
alter table ziti_edge.authenticator_certs alter column authenticator_id set not null;

-- remove identity id column, now available on ziti_edge.authenticators parent
alter table ziti_edge.authenticator_certs drop column identity_id;
