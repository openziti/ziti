...
import (
	...
	apiversion "github.com/netfoundry/ziti-edge/edge/internal/version"
	"fmt"
...
func (er *ErrorResponse) Respond(rc *RequestContext) error {
	...
	if _, err := jsonObj.SetP(apiversion.GetApiVersion(), "meta.apiVersion"); err != nil {
		pfxlog.Logger().WithField("cause", err).Panic("could not set value by path")
	}

	if _, err := jsonObj.SetP(apiversion.GetApiEnrollmentVersion(), "meta.apiEnrolmentVersion"); err != nil {
		pfxlog.Logger().WithField("cause", err).Panic("could not set value by path")
	}
