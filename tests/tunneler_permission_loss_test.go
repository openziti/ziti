...
const (
	...
	ertPostureService   = "ert-posture-svc"
	ertPosturePort      = 8693
)
...
func newTunnelerDialFixture(ctx *TestContext, oidcHost bool) *tunnelerDialFixture {
	...
	service1Watcher.waitForTerminators(10 * time.Second)
	service2Watcher.waitForTerminators(10 * time.Second)
...
func (f *tunnelerDialFixture) requireService1Revoked() {
	...
	// either the intercept listener is gone or the router refuses the dial before a circuit is created
	ctx.Req.Eventually(func() bool {
		return newDialRefused(f.addr1)
	}, 10*time.Second, 250*time.Millisecond, "expected service1 dials to be refused")
...
// Test_TunnelerCircuitCloseOnPostureCheckAdded verifies that adding a posture check to the dial
// policy granting the router closes its existing circuits, since routers cannot satisfy posture checks.
// Membership does not change, so this exercises the dial-policies-changed path rather than access lost.
func Test_TunnelerCircuitCloseOnPostureCheckAdded(t *testing.T) {
	ctx := NewTestContext(t)
	defer ctx.Teardown()
	ctx.StartServer()
	ctx.RequireAdminManagementApiLogin()
	...
	postureRole := eid.New()
	ctx.AdminManagementSession.requireNewPostureCheckDomain(s("example.com"), s(postureRole))

	f := newTunnelerDialFixture(ctx, false)

	f.dialPolicy1.postureCheckRoles = s("#" + postureRole)
	ctx.AdminManagementSession.requirePatchEntity(f.dialPolicy1, "postureCheckRoles")
...
func Test_TunnelerCircuitDeniedWithoutDialPolicy(t *testing.T) {
	...
	reply, err := protobufs.MarshalTyped(request).WithTimeout(5 * time.Second).SendForReply(ctrlCh.GetDefaultSender())
	ctx.Req.NoError(err)
...
// Test_TunnelerDialDeniedByPostureCheck verifies that a dial policy carrying a posture check does not
// grant a tunneler access, since routers submit no posture data: the intercepted connection is closed
// without a circuit being established.
func Test_TunnelerDialDeniedByPostureCheck(t *testing.T) {
	ctx := NewTestContext(t)
	defer ctx.Teardown()
	ctx.StartServer()
	ctx.RequireAdminManagementApiLogin()
	...
	service := ctx.newService(s(svcRole), nil)
	service.Name = ertPostureService
	...
	ctx.AdminManagementSession.requireNewServiceEdgeRouterPolicy(s("#all"), s("#all"))
	ctx.AdminManagementSession.requireNewServicePolicy("Bind", s("#"+svcRole), s("#"+hostRole), nil)
	ctx.AdminManagementSession.requireNewServicePolicy("Dial", s("#"+svcRole), s("#all"), s("#"+postureRole))
	...
	ctx.CreateEnrollAndStartTunnelerEdgeRouterWithCfgTweaks(func(cfg *routerEnv.Config) {
		for _, l := range cfg.Listeners {
			if l.Name == "tunnel" {
				if opts, ok := l.Options["options"].(map[interface{}]interface{}); ok {
					opts["services"] = []interface{}{
						fmt.Sprintf("%s:%d", ertPostureService, ertPosturePort),
					}
				}
			}
		}
	...
	newTestServer(listener, func(conn *testServerConn) error {
		for {
			name, eof := conn.ReadString(1024, time.Minute)
			if eof {
				return nil
	...
	// the intercept is still installed, since intercept setup is driven by policy membership, but the
	// dial itself must be refused
	conn := requireDialWithRetry(ctx, fmt.Sprintf("127.0.0.1:%d", ertPosturePort), 15*time.Second)
	defer func() { _ = conn.Close() }()

	ctx.Req.NoError(conn.SetDeadline(time.Now().Add(5 * time.Second)))
	_, _ = conn.Write([]byte(eid.New()))
	n, err := conn.Read(make([]byte, 64))
	ctx.Req.Error(err, "expected the tunneler to close the connection without a reply, got %d bytes", n)
	var netErr net.Error
	ctx.Req.False(errors.As(err, &netErr) && netErr.Timeout(), "connection was neither answered nor closed")
}
...
func Test_TunnelerCircuitDeniedForDisabledIdentity(t *testing.T) {
	...
	// sanity: the dial is granted while the identity is enabled
	sendDial := func() (*channel.Message, error) {
		request := &edge_ctrl_pb.CreateTunnelCircuitV2Request{ServiceName: service.Name}
		return protobufs.MarshalTyped(request).WithTimeout(5 * time.Second).SendForReply(ctrlCh.GetDefaultSender())
	}
	reply, err := sendDial()
	ctx.Req.NoError(err)
	ctx.Req.Equal(int32(edge_ctrl_pb.ContentType_CreateTunnelCircuitV2ResponseType), reply.ContentType, "dial should succeed while enabled")
	...
	// the controller must now deny the dial with an access-denied error. The router's control channel
	// stays up (it authenticates by certificate, not by the disabled identity's session), so poll
	// until the disable has been applied to the identity store the handler reads.
	ctx.Req.Eventually(func() bool {
		reply, err := sendDial()
		if err != nil {
			return false
