...
func Test_ServiceConfigTargetValidation(t *testing.T) {
	...
	t.Run("service with default-target config should succeed", func(t *testing.T) {
		ctx.testContextChanged(t)
		ct := ctx.newConfigType()
		ct.Id = ctx.AdminManagementSession.requireCreateEntity(ct)
		config := ctx.AdminManagementSession.requireCreateNewConfig(ct.Id, map[string]interface{}{})
	...
	t.Run("service with other-target config should fail", func(t *testing.T) {
		ctx.testContextChanged(t)
		ct := ctx.newConfigType()
		ct.Target = util.Ptr("other")
		ct.Id = ctx.AdminManagementSession.requireCreateEntity(ct)
		config := ctx.AdminManagementSession.requireCreateNewConfig(ct.Id, map[string]interface{}{})
		svc := ctx.newService(nil, s(config.Id))
		resp := ctx.AdminManagementSession.createEntity(svc)
...
func Test_IdentityServiceConfigTargetValidation(t *testing.T) {
	...
	t.Run("identity service config override with default-target config should succeed", func(t *testing.T) {
		ctx.testContextChanged(t)
		ct := ctx.newConfigType()
		ct.Id = ctx.AdminManagementSession.requireCreateEntity(ct)
		config := ctx.AdminManagementSession.requireCreateNewConfig(ct.Id, map[string]interface{}{})
	...
	t.Run("identity service config override with other-target config should fail", func(t *testing.T) {
		ctx.testContextChanged(t)
		ct := ctx.newConfigType()
		ct.Target = util.Ptr("other")
		ct.Id = ctx.AdminManagementSession.requireCreateEntity(ct)
