...
func Test_SDK_SubscriptionReconciliation(t *testing.T) {
	...
	// A service known via polling before the subscribe, exercising the seamless poll-to-push
	// handoff (it must not re-emit an added event when the snapshot confirms it).
	ctx.AdminManagementSession.requireNewService(s(serviceRole), nil)
	...
	t.Run("subscribe with a capable router uses push", func(t *testing.T) {
		...
		// The poll-to-push transition is seamless: services already known from polling (like
		// initialService) do not re-emit added events. Push delivery is asserted with a service
		// created after the subscribe.
		...
		ctx.Req.True(waitForServiceAdded(pushService.Name, 15*time.Second),
			"service created after subscribing should arrive via push")
	})
