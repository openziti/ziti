...
func Test_ManagedConfig_ListenerUpdatePublishesToController(t *testing.T) {
	...
	ctx.Req.Eventually(func() bool {
		...
		listeners := ctrlRouter.GetLinkListeners()
		return len(listeners) == 1 &&
			len(listeners[0].Groups) == 1 &&
			listeners[0].Groups[0] == "a"
	...
	// The controller's link listener set should reflect the new groups.
	ctx.Req.Eventually(func() bool {
		ctrlRouter := ctx.fabricController.GetNetwork().Router.GetConnected(r.Router.GetRouterId().Token)
		if ctrlRouter == nil {
			return false
...
// link.Subsystem — the old listener slice is captured and closed
