...
func Test_PostureCheck_SDK_MAC_OIDC(t *testing.T) {
	...
	targetMacAddr := "00:1B:21:AE:43:97"

	validMacAddr := targetMacAddr

	invalidMacAddr := "3A:7C:92:4F:11:B6"

	postureCheckDetail, err := adminManagementApi.CreatePostureCheckMac([]string{targetMacAddr}, []string{postureCheckRoleAttr})
	ctx.Req.NoError(err)
	...
	t.Run("a client sdk", func(t *testing.T) {
		...
		t.Run("can provide valid posture data and dial the service", func(t *testing.T) {
			...
			currentReportedMacs := validMacAddr
			postureCache.SetMacProviderFunc(func() []string {
				return []string{currentReportedMacs}
			})
