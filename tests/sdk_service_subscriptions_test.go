...
func Test_SDK_ServiceSubscriptions(t *testing.T) {
	...
	t.Run("subscribe and receive service added via push", func(t *testing.T) {
		ctx.testContextChanged(t)

		err = clientCtx.SubscribeToServiceUpdatesFromRouter()
		ctx.Req.NoError(err)
		ctx.Req.True(clientCtx.IsPushSubscriptionActive(), "push should be active after subscribing")

		// The poll-to-push transition is seamless: the snapshot confirms the polled view without
		// re-emitting added events for services the SDK already knows. Push delivery is asserted
		// with a service created after the subscribe.
		pushService := ctx.AdminManagementSession.requireNewService(s(serviceRole), nil)

		select {
		case svc := <-serviceAdded:
			ctx.Req.Equal(pushService.Name, *svc.Name)
		case <-time.After(10 * time.Second):
			t.Fatal("timeout: expected ServiceAdded event via router push for a service created after subscribing")
		}
	})
