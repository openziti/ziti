...
func Test_SDK_ServiceConfigPush(t *testing.T) {
	...
	t.Run("snapshot carries the config body only for the referencing service", func(t *testing.T) {
		...
		// The poll-to-push handoff is seamless: services already known from polling arrive as
		// changed events (when the pushed detail enriches them) or no event at all (when
		// identical), so the snapshot's effect is asserted on the SDK's stored view.
		ctx.Req.Eventually(func() bool {
			storedA, found := clientCtx.GetService(serviceA.Name)
			return found && configHostname(storedA, configType.Name) == "v1.example.com"
		}, 10*time.Second, 250*time.Millisecond,
			"serviceA references config1, so its stored view should carry the body after the push snapshot")

		storedB, found := clientCtx.GetService(serviceB.Name)
		ctx.Req.True(found, "serviceB should be retrievable via GetService")
		ctx.Req.NotContains(storedB.Config, configType.Name, "serviceB references no config")
