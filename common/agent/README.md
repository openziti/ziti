...
- **App capabilities** are owned by the embedding application. The package does
  not interpret them, it just passes the strings through. ziti registers its
  own strings (e.g. `ziti.something`) at startup.
...
2. Add its canonical name to the `agentCapabilityNames` table. The string is the
   ...
   var agentCapabilityNames = map[int]string{
       ...
       AgentSomethingNew:      "something.new",
   }
   ```
...
3. Mark it active only when its handler is registered, by calling
   ...
   `RegisterLogLevelHandlers` does for `AgentLoggingSlogLevels`). That is what
...
```go
if opts.HasAgentCapability(agent.AgentLoggingSlogLevels) { ... }
```
...
App capabilities are strings only, with no bit position. Registering after the
listener has started panics, because the advertised set must stay consistent
across every connection. Clients check an app capability by name:
