...
// All three callbacks must be supplied. Once registered, the agent advertises
// logging.slog-levels and the v2 log-level channel commands are bound on every
// agent channel; the framed log-level commands route through the same
// callbacks. It must be called before the agent listener starts; calling it
// afterward panics, so the advertised capability set stays consistent across
// every connection.
func RegisterLogLevelHandlers(cbs LogLevelCallbacks) error {
	...
	defer capsMu.Unlock()
	if capsFrozen {
		panic("agent: RegisterLogLevelHandlers called after the agent listener started")
	}
	cb := cbs
	logLevelCallbacks = &cb
	activeCaps[AgentLoggingSlogLevels] = true
	return nil
}
