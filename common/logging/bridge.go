...
func (slogBridge) Fire(e *logrus.Entry) error {
	...
	r := slog.NewRecord(e.Time, level, e.Message, pc)
...
// returns the bootstrap discard handler; post-Configure it returns
...
// SyncEmit (which bypasses the queue under the same mutex the drain uses).
// Otherwise, it falls back to Handle, which is already synchronous for any
// non-async handler.
//
