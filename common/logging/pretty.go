...
// trimmed from function names, and color decided by the PFXLOG_USE_COLOR
// environment variable when set, otherwise by whether out is a terminal.
func DefaultPrettyOptions(out io.Writer) *PrettyOptions {
	now := time.Now()
	return &PrettyOptions{
		...
		UseColor:        useColor(out),
	}
}

// useColor decides the default color setting: PFXLOG_USE_COLOR wins when it
// parses as a bool, otherwise color is on iff out is a character device.
...
// DefaultPrettyOptions(out); out defaults to os.Stderr when nil.
func NewPrettyHandler(out io.Writer, opts *PrettyOptions) *PrettyHandler {
	if out == nil {
		out = os.Stderr
	}
	if opts == nil {
		opts = DefaultPrettyOptions(out)
	}
