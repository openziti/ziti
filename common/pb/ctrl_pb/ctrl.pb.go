...
//
// The same vocabulary is used by the router.link gcMode config field,
// which adds a 'preserve' value to mean "never act."
...
type LinkStaleReport struct {
	...
	// iteration is the link incarnation this verdict was computed against. A link
	// id is reused across re-dials, so a verdict is only valid for the iteration
	// that produced it.
...
func (x *LinkStaleReport) GetIteration() uint32 {
	if x != nil {
		return x.Iteration
	}
	return 0
...
const file_ctrl_proto_rawDesc = "" +
	...
	"\titeration\x18\x05 \x01(\rR\titeration\"\xa2\x01\n" +
