		...
		if valueInMap == nil {
			...
			if valueInMap.PolicyType != servicePolicy.PolicyType {
				valueInMap.Identities.IterCb(func(key string, _ struct{}) {
					rdm.withLockedIdentity(true, key, func(identity *Identity) {
						identity.removedFromPolicy(valueInMap)
						identity.addedToPolicy(updatedValue)
					})
				})
			}
