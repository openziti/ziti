...
// Generates a http.Handler that interrogates a http.Request and routes them to WebHandlers. The selected WebHandler
// is added to the context with a key of WebHandlerContextKey. Each DemuxFactory implementation must define
// its own behaviors for an unmatched http.Request.
type DemuxFactory interface {
	Build(handlers []WebHandler) (http.Handler, error)
}

// A DemuxFactory that routes http.Request requests to a specific WebHandler from a set of WebHandler's by URL path prefixes.
// a http.Handler for NoHandlerFound can be provided to specify behavior to perform when a WebHandler is not selected. By default
// an empty response with a http.StatusNotFound (404) will be sent.
...
// Provide an empty 404 for unmached path if NoHandlerFound is nil, otherwise call and defer to NoHandlerFound.
func (factory *PathPrefixDemuxFactory) noHandlerFound(writer http.ResponseWriter, request *http.Request) {
	if factory.NoHandlerFound != nil {
		//defer to externally specified http.Handler fun
