...
func (s *stubCtrlChannel) IsClosed() bool                      { return s.closed }
...
// seedSender installs rtx directly, bypassing GetOrCreate so tests can set up a starting state
// without exercising the code under test.
func seedSender(m *routerTxMap, rtx *RouterSender) {
	m.internalMap.Set(rtx.Router.Id, rtx)
}
...
func Test_GetOrCreate_createsWhenAbsent(t *testing.T) {
	req := require.New(t)
	m := newRouterTxMap()
	ch := &stubCtrlChannel{}
...
func Test_GetOrCreate_reusesSenderWhenChannelMatches(t *testing.T) {
	req := require.New(t)
	m := newRouterTxMap()
	ch := &stubCtrlChannel{}
	existing := newStubSender("r1", ch, true)
	seedSender(m, existing)
...
func Test_GetOrCreate_replacesAndStopsStaleChannelSender(t *testing.T) {
	req := require.New(t)
	m := newRouterTxMap()
...
func Test_GetOrCreate_concurrentCallsCreateOneSender(t *testing.T) {
	req := require.New(t)
	m := newRouterTxMap()
	ch := &stubCtrlChannel{}
...
func Test_RangeEdge_visitsOnlyEdgeSenders(t *testing.T) {
	req := require.New(t)
	m := newRouterTxMap()
	seedSender(m, newStubSender("edge-1", &stubCtrlChannel{}, true))
	seedSender(m, newStubSender("transit-1", &stubCtrlChannel{}, false))
	seedSender(m, newStubSender("edge-2", &stubCtrlChannel{}, true))
