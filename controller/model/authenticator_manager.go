...
func (self *AuthenticatorManager) ReadByUsername(username string) (*Authenticator, error) {
	...
	if err := self.readEntityWithIndex("name", []byte(username), index, modelEntity); err != nil {
...
func (self *AuthenticatorManager) ReadByFingerprint(fingerprint string) (*Authenticator, error) {
	...
	if err := self.readEntityWithIndex("name", []byte(fingerprint), index, modelEntity); err != nil {
		if boltz.IsErrNotFoundErr(err) {
			return nil, nil
		}
		return nil, err
