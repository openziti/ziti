...
func newHttpRouter(provider op.OpenIDProvider, config Config) (*mux.Router, error) {
	...
	serverHandler := op.RegisterLegacyServer(legacyServer, op.AuthorizeCallbackHandler(provider))
	...
	loginRouter := newLogin(config.Storage, legacyServer.AuthCallbackURL(), op.NewIssuerInterceptor(provider.IssuerFromRequest))
