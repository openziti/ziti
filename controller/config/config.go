					...
					if value, found := submap["queueSize"]; found {
						if intVal, ok := value.(int); ok {
							...
							if v < BackgroundQueueMinSize {
								return nil, errors.Errorf("invalid value %v for command.background.queueSize, must be at least %d", value, BackgroundQueueMinSize)
							}
							if v > BackgroundQueueMaxSize {
								return nil, errors.Errorf("invalid value %v for command.background.queueSize, must be at most %d", value, BackgroundQueueMaxSize)
							}
