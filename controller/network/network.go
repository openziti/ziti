...
func (network *Network) QueueRouterConnect(r *model.Router) error {
	...
	// Registering here, before the deferred setup builds this router's links, is what LinkManager.repairDest
	// relies on: it re-reads the connected map once a link is in the table, so a router whose link build ran
	// too early must already be registered by then.
