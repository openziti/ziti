...
// staleLinkQueryConcurrency bounds how many routers are queried at once, matching
// the other validation sweeps.
const staleLinkQueryConcurrency = 10
...
// asked, the reason why: offline, too old to answer, or outside the filter.
// Without this a link is reported partial with nothing to distinguish them, and
// only the last is fixed by changing the command.
...
func (n *Network) runStaleLinkValidation(
	...
	// arrive.
	collector := newStaleLinkReportCollector(linkMap)

	// An endpoint outside the filter is never queried, so its side stays unknown
	// and the link is reported partial. Record that as a reason too: otherwise
	// the three causes of partial — offline, too old, and never asked — are
	// indistinguishable in the output, and only this one is fixed by widening
	// the filter.
	filtered := make(map[string]struct{}, len(routers))
	for _, r := range routers {
		filtered[r.Id] = struct{}{}
	}

	unqueryable := map[string]string{}
	for _, link := range linkMap {
		for _, routerId := range []string{link.Src.Id, link.DstId} {
			if _, inFilter := filtered[routerId]; !inFilter {
				unqueryable[routerId] = fmt.Sprintf(
					"router %s is outside the filter and was not queried; --gc needs both endpoints", routerId)
			}
		}
	}

	// Fan out to routers in the filter set that can actually answer, a bounded
	// number at a time: each query carries a minute-long reply timeout, so an
	// unbounded fan-out would leave one outstanding request per router on the
	// wire at once. Routers that can't answer are recorded with a reason
	// instead, so the partial verdicts they cause say what to fix.
	sem := concurrenz.NewSemaphore(staleLinkQueryConcurrency)
...
type staleLinkReportCollector struct {
	// links is the sweep's link map, read-only after construction. Held so a
	// report can be attributed to a side using the controller's own view of the
	// link rather than the side the report names.
	links    map[string]*model.Link
	mu       sync.Mutex
	verdicts map[string]*linkVerdicts
}

func newStaleLinkReportCollector(links map[string]*model.Link) *staleLinkReportCollector {
	return &staleLinkReportCollector{
		links:    links,
		verdicts: make(map[string]*linkVerdicts, len(links)),
	}
}

// sideFor resolves which side of link the reporting router speaks for. The
// controller knows both endpoints, so it decides: a report names a side, but
// only the link map says whether the sender is that endpoint. Reports from a
// router that is neither endpoint are refused.
func sideFor(link *model.Link, routerId string) (ctrl_pb.StaleLinkSide, bool) {
	switch routerId {
	case link.Src.Id:
		return ctrl_pb.StaleLinkSide_StaleLinkSideDialer, true
	case link.DstId:
		return ctrl_pb.StaleLinkSide_StaleLinkSideListener, true
	}
	return 0, false
}

// Record files one router's report against the link it names. Attribution is by
// sender: --gc acts when both endpoints agree, and taking the side from the
// report would let a single router satisfy that on its own by sending two.
func (c *staleLinkReportCollector) Record(routerId string, report *ctrl_pb.LinkStaleReport) {
	link, found := c.links[report.LinkId]
	if !found {
		return // not a link this sweep reports on
	}

	side, ok := sideFor(link, routerId)
	if !ok {
		pfxlog.Logger().WithField("routerId", routerId).WithField("linkId", report.LinkId).
			Warn("ignoring stale-link report from a router that is not an endpoint of the link")
		return
	}
	if side != report.Side {
		pfxlog.Logger().WithField("routerId", routerId).WithField("linkId", report.LinkId).
			WithField("reportedSide", report.Side).WithField("side", side).
			Warn("stale-link report names a side the controller does not agree with; using the controller's")
	}
	...
	applyStaleReport(v, side, report)
}
...
func (n *Network) collectStaleReports(
	router *model.Router,
	mode mgmt_pb.StaleLinkMatchMode,
	record func(routerId string, report *ctrl_pb.LinkStaleReport),
) {
	...
	for _, report := range resp.Reports {
		record(router.Id, report)
	}
}

// applyStaleReport files report's verdict under side, which the caller resolves
// from the reporting router rather than from the report.
func applyStaleReport(v *linkVerdicts, side ctrl_pb.StaleLinkSide, report *ctrl_pb.LinkStaleReport) {
