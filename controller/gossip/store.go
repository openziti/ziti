...
func (sm *stateMap) markBroadcastDropped() {
	if sm.metrics != nil {
		sm.metrics.broadcastDropped.Mark(1)
	}
}
...
func (sm *stateMap) registerMetrics(reg metrics.Registry) {
	...
	// funcGauge registers a functional gauge, disposing any existing gauge of the
	// same name first. HA controllers call InitGossipStore (and thus Register ->
	// registerMetrics) twice: once for the noop store in NewNetwork, then again
	// for the real raft-backed store. FuncGauge is get-or-create and keeps the
	// FIRST closure, which would leave these gauges bound to the dead noop store
	// (reading 0 forever). Disposing first makes the latest (active) store win.
...
// diagStaleRejectLast rate-limits the stale-rejection diagnostic to ~one example
// per second across the whole store.
var diagStaleRejectLast atomic.Int64

// logStaleReject emits a rate-limited DIAGNOSTIC when an apply is rejected because
// the stored entry's version is >= the incoming version, recording whether the
// epochs differ. TEMPORARY (#12B): present to determine whether the large-gap
// non-convergence is driven by post-restart (new-epoch) entries losing to
// higher-versioned pre-restart (old-epoch) state. Remove once diagnosed.
func (sm *stateMap) logStaleReject(existing, incoming *entry) {
	now := time.Now().UnixNano()
	last := diagStaleRejectLast.Load()
	if now-last < int64(time.Second) || !diagStaleRejectLast.CompareAndSwap(last, now) {
		return
	}
	pfxlog.Logger().
		WithField("storeType", sm.name).
		WithField("owner", incoming.Owner).
		WithField("key", incoming.Key).
		WithField("incomingVersion", incoming.Version).
		WithField("storedVersion", existing.Version).
		WithField("storedTombstone", existing.Tombstone).
		WithField("sameEpoch", bytes.Equal(existing.Epoch, incoming.Epoch)).
		WithField("incomingEpoch", fmt.Sprintf("%x", incoming.Epoch)).
		WithField("storedEpoch", fmt.Sprintf("%x", existing.Epoch)).
		Info("DIAGNOSTIC: gossip apply rejected as stale (#12B)")
}
...
func (sm *stateMap) applyEntryLocked(od *ownerData, incoming *entry) (wasSet, isCreate bool) {
	...
	if ok && existing.Version >= incoming.Version {
		sm.logStaleReject(existing, incoming)
		return false, false
	}
