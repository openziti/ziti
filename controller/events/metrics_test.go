...
func Test_FilterMetrics_MappedNames(t *testing.T) {
	tests := []struct {
		...
		{
			// A raw-name filter selects nothing: the per-field gate matches against the emitted name, so
			// such a filter chooses no field and the event is discarded for having none. Rejecting it here
			// reaches that outcome without the work of building an event that cannot survive.
