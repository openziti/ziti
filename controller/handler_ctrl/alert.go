...
func (self *alertHandler) ContentType() int32 {
	return int32(ctrl_pb.ContentType_AlertType)
}

func (self *alertHandler) HandleReceive(msg *channel.Message, ch channel.Channel) {
	alertMsg := &ctrl_pb.Alert{}
	if err := proto.Unmarshal(msg.Body, alertMsg); err == nil {
		...
		if selfType, ok := alert.RelatedEntities["self"]; ok {
			...
			delete(alert.RelatedEntities, "self")
		}
