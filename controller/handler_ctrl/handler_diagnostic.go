...
//
// Controller-side counterpart to router/env/handler_diagnostic.go. Wraps every
// ctrl-channel receive handler the controller registers for a router connection
// with a timer and logs slow handlers on the controller rx path. Added to see
// whether the back-pressure / slow-handler behavior tracked in link-bugs.md #2
// also shows up on the controller side.
//
// Remove this file (and the WithSlowHandlerDiagnostic call site in
// controller/handler_ctrl/accept.go), along with the router-side diagnostic,
// once the ctrl-channel back-pressure root cause is resolved.
