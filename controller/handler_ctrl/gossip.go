...
// Applying router gossip is deferred to a pool, so every one of these handlers rechecks that the connection
// its message arrived on is still the router's current one before applying it.
//
// The connection can be given up while the work waits, and a restarting router's hello runs the epoch cleanup
// that discards the lifetime the queued entries belong to. Applying them then puts back what the cleanup just
// removed, and the store's version check cannot tell: it orders by version, and the epoch only gates deletion.
//
// This narrows the window rather than closing it. The cleanup runs before the connect decision that displaces
// the old connection, so a message applying between those two still gets through; that leaves the redial's
// hello to clean up again, which it does.
//
...
func (h *ctrlGossipDeltaHandler) HandleReceive(msg *channel.Message, _ channel.Channel) {
	...
	if err := h.network.GetGossipApplyPool().QueueOrError(func() {
		if !h.network.IsCurrentConnection(h.r) {
			log.Info("router connection is no longer current, dropping gossip delta")
			return
		}
...
func (h *ctrlGossipDigestResponseHandler) HandleReceive(msg *channel.Message, _ channel.Channel) {
	...
	if err := h.network.GetGossipApplyPool().QueueOrError(func() {
		if !h.network.IsCurrentConnection(h.r) {
			log.Info("router connection is no longer current, dropping gossip digest response")
			return
		}
