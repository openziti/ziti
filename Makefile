...
ziti-tunnel-common:
	...
	cd release; CGO_ENABLED=$(CGO_ENABLED) CC=/opt/osxcross/target/bin/o64-clang CXX=/opt/osxcross/target/bin/o64-clang++ gox -cgo -os="darwin" -arch=amd64 $(BUILDFLAGS_DARWIN_AMD64) github.com/netfoundry/ziti-cmd/ziti-tunnel/...
	cd release; CGO_ENABLED=$(CGO_ENABLED) gox -cgo -os="linux" -arch=amd64 $(BUILDFLAGS_LINUX_AMD64) github.com/netfoundry/ziti-cmd/ziti-tunnel/...
	cd release; CGO_ENABLED=$(CGO_ENABLED) CC=arm-linux-gnueabihf-gcc gox -cgo -os="linux" -arch=arm $(BUILDFLAGS_LINUX_ARM) github.com/netfoundry/ziti-cmd/ziti-tunnel/...
	cd release; CGO_ENABLED=$(CGO_ENABLED) CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ gox -cgo -os="windows" -arch=amd64 $(BUILDFLAGS_WINDOWS_AMD64) github.com/netfoundry/ziti-cmd/ziti-tunnel/...
