...
func (exporter Exporter) GetPostureChecks() ([]map[string]interface{}, error) {
	...
	return exporter.getEntities(
		...
		func() (int64, error) {
			limit := int64(1)
			resp, err := exporter.client.API.PostureChecks.ListPostureChecks(&posture_checks.ListPostureChecksParams{Limit: &limit}, nil)
			if err != nil {
				return -1, err
			}
		...
		func(offset *int64, limit *int64) ([]interface{}, error) {
			resp, _ := exporter.client.API.PostureChecks.ListPostureChecks(&posture_checks.ListPostureChecksParams{Limit: limit, Offset: offset}, nil)
