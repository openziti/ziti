...
func (exporter Exporter) GetConfigs() ([]map[string]interface{}, error) {
	...
	return exporter.getEntities(
		...
		func() (int64, error) {
			limit := int64(1)
			resp, err := exporter.client.API.Config.ListConfigs(&config.ListConfigsParams{Limit: &limit}, nil)
			if err != nil {
				return -1, err
			}
		...
		func(offset *int64, limit *int64) ([]interface{}, error) {
			resp, _ := exporter.client.API.Config.ListConfigs(&config.ListConfigsParams{Limit: limit, Offset: offset}, nil)
		...
		func(entity interface{}) (map[string]interface{}, error) {
			...
			configType, lookupErr := ascode.GetItemFromCache(exporter.configTypeCache, *item.ConfigTypeID, func(id string) (interface{}, error) {
				return exporter.client.API.Config.DetailConfigType(&config.DetailConfigTypeParams{ID: id}, nil)
			})
			if lookupErr != nil {
