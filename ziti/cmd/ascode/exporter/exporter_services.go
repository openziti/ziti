...
func (exporter Exporter) GetServices() ([]map[string]interface{}, error) {
	...
	return exporter.getEntities(
		...
		func() (int64, error) {
			limit := int64(1)
			resp, err := exporter.client.API.Service.ListServices(&service.ListServicesParams{Limit: &limit}, nil)
			if err != nil {
				return -1, err
			}
		...
		func(offset *int64, limit *int64) ([]interface{}, error) {
			resp, err := exporter.client.API.Service.ListServices(&service.ListServicesParams{Limit: limit, Offset: offset}, nil)
			if err != nil {
				return nil, err
			}
		...
		func(entity interface{}) (map[string]interface{}, error) {
			...
			for _, c := range item.Configs {
				configDetail, lookupErr := ascode.GetItemFromCache(exporter.configCache, c, func(id string) (interface{}, error) {
					return exporter.client.API.Config.DetailConfig(&config.DetailConfigParams{ID: id}, nil)
				})
				if lookupErr != nil {
