...
func (exporter Exporter) GetEdgeRouterPolicies() ([]map[string]interface{}, error) {
	...
	return exporter.getEntities(
		...
		func() (int64, error) {
			limit := int64(1)
			resp, err := exporter.client.API.EdgeRouterPolicy.ListEdgeRouterPolicies(&edge_router_policy.ListEdgeRouterPoliciesParams{Limit: &limit}, nil)
			if err != nil {
				return -1, err
			}
		...
		func(offset *int64, limit *int64) ([]interface{}, error) {
			resp, err := exporter.client.API.EdgeRouterPolicy.ListEdgeRouterPolicies(&edge_router_policy.ListEdgeRouterPoliciesParams{Limit: limit, Offset: offset}, nil)
			if err != nil {
				return nil, err
			}
