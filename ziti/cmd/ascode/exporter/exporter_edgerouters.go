...
func (exporter Exporter) GetEdgeRouters() ([]map[string]interface{}, error) {
	...
	return exporter.getEntities(
		...
		func() (int64, error) {
			limit := int64(1)
			resp, err := exporter.client.API.EdgeRouter.ListEdgeRouters(&edge_router.ListEdgeRoutersParams{Limit: &limit}, nil)
			if err != nil {
				return -1, err
			}
		...
		func(offset *int64, limit *int64) ([]interface{}, error) {
			resp, err := exporter.client.API.EdgeRouter.ListEdgeRouters(&edge_router.ListEdgeRoutersParams{Limit: limit, Offset: offset}, nil)
			if err != nil {
				return nil, err
			}
