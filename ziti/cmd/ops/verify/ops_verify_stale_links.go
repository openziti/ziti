...
// entirely absent. --gc closes links both endpoints confirm as stale; a link
// is never closed while either endpoint is offline, too old to answer, or
// outside --filter.
...
are closed via the standard LinkFault path. Links with partial info are
never GC'd. An endpoint gives partial info when it is offline, when it is
too old to answer, or when --filter did not select it.

--filter chooses which routers are asked, so a link is only collectable
when the filter covers both of its endpoints. Filtering to one end of a
link reports it, but leaves it partial and never closes it.`,
		Example: `  # report on every link in the network
  ziti ops verify stale-links

  # report on links involving the east routers, without closing anything
  ziti ops verify stale-links --filter 'name contains "east"'

  # close confirmed-stale links; both endpoints must be in the filter
  ziti ops verify stale-links --gc`,
		RunE: action.run,
	}
