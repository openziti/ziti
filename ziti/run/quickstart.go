...
func (o *QuickstartOpts) Run(ctx context.Context) error {
	...
	if o.ControllerPort > 0 {
		...
		p, _ := strconv.ParseInt(helpers.GetCtrlEdgeAdvertisedPort(), 10, 16)
		o.ControllerPort = uint16(p)
	}
	if o.RouterAddress != "" {
	...
	if _, err := os.Stat(dbDir); !os.IsNotExist(err) {
		...
		err = o.CreateControllerConfig(o.ConfigFile)
		if err != nil {
			logrus.Fatal(err)
		}

		if !o.IsHA {
			if ie := o.InitLegacy(); ie != nil {
				return ie
			}
		}
	}
	...
	ctrlUrl := fmt.Sprintf("https://%s:%d", o.ControllerAddress, o.ControllerPort)

	c := make(chan error)
	timeout := 30 * time.Second
	go o.waitForController(c)
	select {
	case <-c:
		//completed normally
	...
	o.runRouter(erConfigFile, curCtx)
	...
	if !o.Routerless {
		...
		case <-r:
			//completed normally
			logrus.Infof("router available at: %s:%d", o.RouterAddress, o.RouterPort)
		case <-time.After(timeout):
			o.cleanupHome()
			cancel()
	...
	if o.ConfigureAndExit {
		cancel()
		ch <- os.Interrupt
		time.Sleep(2 * time.Second) // allow the controller/router to settle out
	}
...
func (o *QuickstartOpts) runRouter(configFile string, ctx context.Context) {
	if o.Routerless {
		return
	}
...
func (o *QuickstartOpts) CreateMinimalPki() {
	...
	pkiIntermediateName := o.scopedName("intermediate-ca")
	expectedIntermediatePath := path.Join(where, "root-ca", "certs", pkiIntermediateName+".cert")
	if _, err := os.Stat(expectedIntermediatePath); err == nil {
		logrus.Infof("%s exists and will be reused", expectedIntermediatePath)
	} else {
		...
		intermediate.SetArgs([]string{
			fmt.Sprintf("--pki-root=%s", where),
			fmt.Sprintf("--ca-name=%s", "root-ca"),
			fmt.Sprintf("--intermediate-name=%s", pkiIntermediateName),
			fmt.Sprintf("--intermediate-file=%s", pkiIntermediateName),
			"--max-path-len=1",
		})
	...
	pkiServerCertName := o.scopedNameOff("server")
	expectedServerCertPath := path.Join(where, pkiIntermediateName, "certs", pkiServerCertName+".cert")
	if _, err := os.Stat(expectedServerCertPath); err == nil {
		logrus.Infof("%s exists and will be reused", expectedServerCertPath)
	} else {
		...
		args := []string{
			fmt.Sprintf("--pki-root=%s", where),
			fmt.Sprintf("--ca-name=%s", pkiIntermediateName),
			fmt.Sprintf("--server-name=%s", o.InstanceID),
			fmt.Sprintf("--server-file=%s", pkiServerCertName),
	...
	pkiClientCertName := o.scopedNameOff("client")
	expectedClientCertPath := path.Join(where, pkiIntermediateName, "certs", pkiClientCertName+".cert")
	if _, err := os.Stat(expectedClientCertPath); err == nil {
		logrus.Infof("%s exists and will be reused", expectedClientCertPath)
	} else {
		...
		client.SetArgs([]string{
			...
			fmt.Sprintf("--client-file=%s", pkiClientCertName),
			fmt.Sprintf("--key-file=%s", pkiServerCertName),
			fmt.Sprintf("--spiffe-id=%s", sid),
		})
...
func (o *QuickstartOpts) waitForController(done chan error) {
	ctrlUrl := o.ControllerHostPort()
	fmt.Printf("Waiting for controller to become ready at: %s\n", ctrlUrl)
...
func (o *QuickstartOpts) CreateControllerConfig(ctrlYaml string) error {
