...
makeDatabase() {
  ...
  _cleanup_init_pid() {
    if [[ -n "${_init_pid:-}" ]]; then
      ...
      # Loop until process is dead or timeout
      local _cleanup_attempts=10
      while (( _cleanup_attempts-- )) && kill -0 "${_init_pid}" 2>/dev/null; do
        sleep 0.5
      done
      
      # If still alive, kill the process group with SIGKILL
      if kill -0 "${_init_pid}" 2>/dev/null; then
        echo "DEBUG: force killing process group ${_init_pid}" >&3
        kill -9 -"${_init_pid}" 2>/dev/null || true
      fi
    fi
  }
  ...
  if (( _attempts )); then
    echo "DEBUG: initialized controller database with ${_attempts} remaining attempts" >&3
    return 0
  else
