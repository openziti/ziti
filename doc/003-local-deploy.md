...
A Postgres installation is required for the Edge components of the Ziti Controller to operate. If an existing Postgres
instance is available, then substitute the connection information in the controller configuration that is generated
in this section.

It is suggested that Postgres be installed via Docker; as outlined below.

If desired, Postgres can be installed on your host machine via your platforms package management system (if available) 
or directly from https://www.postgresql.org/. The Docker approach is highly encouraged for easy tear down. 
...
1. Start a Postgres environment on your local machine (either local installation or Docker container, Docker commands provided below:

    - Install & Run: `docker run --name pg -d -e POSTGRES_PASSWORD=ztpassword -p 5432:5432 postgres`
    - Stop: `docker stop pg`
    - Start: `docker start pg`
    - Verify: `docker ps` see the `postgres` image is running under name `pg`
