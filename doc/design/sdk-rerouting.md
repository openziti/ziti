...
The work pivots on **Phase C**, which separates paths from circuits. Before C,
the phases make today's fused model survivable: A (dialer-side reroute), B1
(terminator-side reroute), and B4 (controller<->router connectivity resilience).
C is the structural change. After C, two tracks build on the path model:
performance (B2, multi-path) and decentralization (D, plus B5 and the B3
residual, co-designed). So the real order is
A -> B1 -> B4 -> C -> {B2 | (D + B5 + B3)}. The letters are names, not a strict
sequence, and may be renamed to something more descriptive later; the ordering
and dependencies above are what matter.
...
- **Phase B5, Source-routed route installation:** the controller stops
  installing routes by reaching every router on the path; instead it signs
  the computed path into a route token that the ingress router forwards
  hop-by-hop, each router installing its own forward-table entry, so a
  circuit establishes as long as one router on the path reaches any
  controller. Pairs with the gossip work (which gives controllers full
  topology knowledge) to enable routing in a partially-connected mesh.
- **Phase C, Path/circuit separation (long-lived paths):** paths become
  reference-counted long-lived resources and circuits become lightweight
  records that point at one (still one circuit per path). This alone makes
  SDK reroute a make-before-break path swap and cleanup self-expiring soft
  state, and it pays the one wire break the rest of the path model needs.
- **Phase D, Path sharing and router-local circuit creation:** rides on
  Phase C additively; many circuits share a path, and routers create
  circuits locally with sub-millisecond, controller-round-trip-free dials.
...
The real order is A -> B1 -> B4 -> C -> {B2 | (D + B5 + B3)}, organized around
the **Phase C** pivot (path/circuit separation, `separate-path-from-circuit.md`).
A, B1, and B4 make today's fused model survivable and build directly on A's
machinery; each ships independently and precedes C. C is the structural change.
The remaining phases build on C, not just on A: B2 (multi-path) is structural
once paths are first-class, and B5 (source-routed install) plus the B3 residual
(cross-controller) join D on the decentralization track. So B2/B3/B5 follow C
despite their B labels; the letters are names, not an order, and may be renamed
more descriptively later.
...
- New circuit state `Limbo` (or extend `Rerouting`) with an associated
  ...
  **Exit, X1 (`TakeoverCircuit` succeeds):** SDK has dialed a new
  ...
  3. Controller acquires the `circuit.Rerouting` CAS guard via the
     ...
     SDK.
  4. Re-validate state under the guard: circuit still reroutable
     (takeover is NOT gated on Limbo, so an active circuit is valid too:
     the SDK detects its own ingress loss before the controller enters
     Limbo, and may also move proactively; the token is the authority,
     not the circuit's state), `token.iteration == circuit.iteration`,
     `token.side == Ingress` (Phase A only does ingress-side takeover;
     rejecting any other side guards against future B1 tokens being
     misrouted to this handler), owning controller is still us. If the
     circuit was in Limbo, its grace timer is stopped on commit.
...
**Stale ingress faults after takeover must be ignored.** Because
takeover can win the race against the old ingress noticing its edge
channel dropped (that's the whole point of not gating takeover on
Limbo), a late `IngressFault` can arrive from the *former* ingress
*after* the circuit has already been relocated and is healthy again.
Acting on it (Limbo or teardown) would poison the live circuit: with
no takeover following, the spurious Limbo's grace timer would tear
down a perfectly good circuit ~10s later. The E1 handler therefore
ignores an `IngressFault` from any router that is not the circuit's
*current* ingress (`circuit.Path.Nodes[0]`).

There is a residual case this router check can't cover: a *same-router*
takeover, where the SDK reattaches to the same ingress via a fresh
edge channel. The old channel's late fault still names the current
ingress, so it looks current. The data-plane side of this is handled
at the router: endpoint cleanup is match-aware
(`UnregisterDestinationIfMatches`), so the old channel's late cleanup
can't remove the forwarder the takeover re-registered at the preserved
ingress address. The control-plane side (the spurious Limbo from the
old channel's fault) needs a **per-generation guard**: stamp the
circuit's `iteration` on the fault at the router and have the
controller drop faults older than the circuit's current iteration.
That requires plumbing the iteration down to the router's
`xgEdgeForwarder` (it doesn't track it today), so it is **deferred**
as a hardening follow-up; until then a same-router takeover racing its
own predecessor's fault detection can still see a spurious Limbo entry.
...
**Implementation note (walking skeleton) and a pre-release gap.** The
candidate confirmation above relies on the numeric hello-header
capability bit, which is authoritative and matches `SupportsConnectV2`.
That part is implemented. What is *not* yet wired is the intended
pre-connect filter: `CommonEdgeRouterProperties.Capabilities` (the REST
string list an ER advertises, whose stated purpose is exactly "select a
router that supports a given feature before connecting") is never
populated by the controller, on either the session ER mapping or the
sessionless `/services/{id}/edge-routers` path. So the SDK's
`edgeRoutersForCapability` string filter returns nothing on real data.

Until that is wired, `GetRerouteCandidates` selects candidates by
connecting (or reusing an already-warmed connection) and checking the
numeric capability on the live channel, skipping routers that fail to
connect or lack the bit. This is correct, but in a mixed-version fleet
it can connect to a router that turns out not to support reroute and
then discard it. **Before release:** map the router's advertised
capabilities to the REST `Capabilities` string list on both ER-list
paths and switch candidate selection to pre-filter via
`edgeRoutersForCapability` before connecting. This pairs with the
mixed-version compatibility work.
...
### Phase B5: Source-routed route installation (controller-connectivity decoupling)

**Motivation.** Separate work is moving the network toward operating in a
less-than-fully-connected mesh. Link inventory is moving to gossip: a
router reports its links to whichever controller it's connected to, and the
cluster shares them; link latency follows; router-managed terminators follow
after that. Once those land, every controller has the *information* to
compute a route for any circuit in the network, including one whose path
touches routers it has no channel to.

What it still lacks is *reach*. Routing today means the controller sends a
`route` message to each router on the path over that router's ctrl channel,
and a controller can't route to a router it isn't connected to. Gossip
closes the knowledge gap; it does nothing for the connectivity gap. A
controller that knows the whole topology still can't install a circuit whose
path runs through a router on the far side of a partition.

**Idea.** Don't have the controller install the route directly. Have it
compute and authorize the path, sign the result into a **route token** (a
controller-signed JWT, reusing the same signing keys and cluster JWKS
verification as the reroute token above), and return that token to the
requesting ingress router. The ingress router installs its own forward-table
entry and forwards the route token to the next hop on the signed path. Each
router verifies the controller's signature, confirms it is itself on the
path, installs its forward-table entry, and forwards to the next signed hop.
The terminating router verifies, performs the dial, and originates the
response, which travels back along the same path to the ingress router and
out to the SDK as the usual `state_connected`.

A router can now establish a circuit whenever it has a channel to *any*
controller plus links to the routers on the computed path, and the path
asserts those links exist by construction. The controller's role shrinks from
"reach every router on the path" to "compute, authorize, sign." This is
source routing with a controller-signed path capability: the signed node
list is the label stack, and the path's own links carry the installation the
controller can no longer deliver itself.

**Why it fits this design.** It's the same primitive the reroute token
already commits to, applied to establishment instead of recovery: a
controller-signed, JWKS-verifiable, iteration-bound token a router acts on
locally after a signature check. Everything the token work buys (signing
infrastructure, per-router local verification, freshness via `iteration`,
`purpose` separation against token confusion) is shared. And it directly
strengthens Phase A's takeover: today the takeover splice (X1) has the
controller send route messages to every router on the *new* path, so a
controller that can't reach a mid-path router can't complete the splice. If
the takeover route were source-routed the same way, the new ingress router
(which the controller must reach, since it sent the takeover request)
forwards the splice along the new path, and takeover works even when the
controller has no channel to the mid-path routers. B5 is the establishment-
and-splice half; the reroute token is the recovery half of one mechanism.

**Relationship to gossip and to B3.** Three separate constraints gate
partial-mesh routing, and they compose:

- *Information*: does a controller know the topology? Answered by the
  gossip work (links, latency, terminators), external to this design.
- *Authority*: may this controller act on this circuit? Answered by B3's
  ownership-transfer protocol.
- *Reach*: can the acting controller install the route on every hop?
  Answered by B5.

Gossip plus B3 plus B5 is the full "route in a partitioned mesh" story; each
alone is insufficient. B5 depends on the gossip work for the information it
signs but is independent of B3 (a single owning controller can source-route
installs); pairing them lifts both the owner-reachable and the
all-routers-reachable constraints at once.

**What has to be solved:**

- **Distributed install and rollback.** This is the real cost. Today route
  installation is controller-orchestrated with an attendance map and
  controller-driven rollback (the C2/C3 commit-after-all-succeed + rollback
  discipline). Source-routed installation moves the transaction onto the
  path: a hop that can't install (resource exhaustion, a link down to the
  next hop, a failed terminator dial) must originate a nack that travels
  back toward the ingress, and every hop that already installed a
  speculative entry must tear it down on the nack. This is a distributed
  two-phase-style commit where today there's a central coordinator: the
  forward pass installs speculatively, the return pass (success or nack)
  commits or rolls back. Designing that failure protocol is the bulk of the
  work. Phase C's soft-state path leases dissolve most of this cost: if
  speculatively-installed hop state is a lease that self-expires unless
  confirmed, a partial install just evaporates, which demotes the explicit
  nack from a correctness requirement to a fast-path optimization (free the
  resources in seconds rather than waiting out the lease). B5 can adopt the
  setup-confirmation lease on its own, ahead of the full path/circuit split,
  and get that de-risking early.
- **Establishment latency.** Direct route messages fan out from the
  controller to all hops in parallel; source-routed install is serial along
  the path (ingress -> ... -> terminator, dial, then back). For the common
  short path (two or three routers) this is comparable and avoids controller
  round-trips to distant routers; for long paths it adds serial hop latency.
  Acceptable given paths are usually short, but worth measuring.
- **Lifecycle still needs a control path.** B5 decouples *establishment*
  connectivity. Fault reporting, unroute/teardown, and fault-driven reroute
  currently also flow controller<->router directly. To fully operate in a
  partial mesh those need the same treatment: faults reach *some* controller
  (via the router's connected controller, propagated by gossip/ownership)
  and unroute/reroute become themselves source-routed signed tokens. B5 is
  the establishment member of a broader source-routed control plane; the
  reroute token is already its recovery member. Scope B5 to establishment
  and sequence the teardown/fault half explicitly rather than assuming
  establishment alone delivers full partial-mesh operation.
- **Security.** The token must bind the full ordered node list so a
  compromised or buggy ingress can't reroute the circuit through arbitrary
  routers; each hop verifies both the signature and its own position,
  forwarding only to the next signed hop. Replay is bounded by the same
  `iteration` freshness the reroute token uses, so a captured route token
  can't reinstall a torn-down circuit. The dial authorization the
  terminating router acts on is a controller-signed claim in the token
  (identity + service), exactly like the reroute token's
  `serviceId`/`identityId`, so the terminator dials on controller authority,
  not on trust of the forwarding routers.
- **Capability gating and fallback.** A `RouterCapabilitySourceRoute` bit
  (name TBD) on the router hello; the controller source-routes only when
  every router on the computed path advertises it, and falls back to direct
  route messages otherwise. Same mixed-version pattern as the rest of this
  design.

**Relationship to Phases C and D.** B5 is a stepping stone toward the path
model, not a replacement. Phase D makes routers create circuits *locally*
off long-lived paths with RDM-delegated authorization and no controller
round-trip at all. B5 keeps the controller in the loop for computation and
authorization but removes it from installation reach. The
terminator-dials-off-a-signed-claim mechanic B5 introduces is the same
authorization-delegation shape Phase D generalizes; building B5 first
exercises signed, router-executed authorization on the establishment path
without committing to Phase C's wire-protocol break and path/circuit split.

**Effort.** Comparable to B3, larger than B4. Signing/verification reuses A's
token machinery, but the distributed install-and-rollback protocol is
genuinely new control-plane surface, and the lifecycle half (faults,
unroute, reroute over the same source-routed path) is a follow-on beyond
establishment. Not on the fastest track; build it when the gossip work has
landed and partial-mesh operation is the active goal.

### Phase C: Path/circuit separation (long-lived paths)

The first half of the strategic destination, and the foundation the rest of
the long-lived-path model builds on. A **Path** becomes a controller-managed,
long-lived, reference-counted resource and a **Circuit** becomes a
lightweight record that points at one. Phase C does the separation itself;
Phase D adds sharing and router-local creation on top. Only Phase C is needed
to unlock the reroute and cleanup wins this design cares about.

Paths become first-class objects; the controller still creates every circuit;
each path still backs exactly one circuit (reference count 0 or 1). This is
the smallest change that makes a path an independently-built,
independently-leased object, which is all that make-before-break reroute and
soft-state cleanup require.

**One wire break, paid once.** The expensive, disruptive part of the
path/circuit split is the forwarder data-model change and the new
path-lifecycle wire messages. Phase C pays that break, and must design it as
the *target* shape so Phase D rides on it additively: the path reference
count is pinned to 0-or-1 in C and allowed to grow to N in D, and
router-local creation is a later additive message, not a re-break. Sequencing
Phase C earlier therefore does not add a second wire break, it moves the
single break earlier (see Sequencing notes).

**The split:**

- A **Path** becomes a controller-managed, long-lived resource:
  `{pathId, [routers], [links], srcRouter, dstRouter}`. Leased (see Path
  lifecycle) and buildable to completion before a circuit switches onto it.
- A **Circuit** becomes lightweight: `{circuitId, pathId, terminator
  binding, ingressXgressId, egressXgressId}`. In Phase C exactly one circuit
  references a path; the many-circuits-per-path case is Phase D.
- The controller still creates circuits (router-local creation is Phase D).
  The change is *how routes are stored, installed, and torn down*, not *who
  may create a circuit*, which keeps Phase C's blast radius contained.

**What Phase C enables:**

- **SDK reroute becomes a make-before-break path swap.** The circuit
  keeps pointing at the live old path while the new path is built to
  completion; only once the new path is confirmed does the circuit
  switch to it, and the switch is a single pointer update
  (`circuit.pathId = newPathId`). No new routes get installed if the
  target path was already provisioned. This dissolves the entire class
  of hazards Phase A has to hand-guard against: the C2/C3
  commit-after-all-routes-succeed + rollback discipline, and the
  "don't mutate `circuit.Path` before install" rule, exist *only*
  because paths and circuits are fused and reroute must mutate the live
  circuit in place. With paths separate, a failed new-path setup is
  handled by simply not swapping; the circuit never left the old path,
  so there is no corrupted baseline to repair.
- **Cleanup becomes soft-state.** Path state on a router is a lease, not
  a permanent installation. Speculative path state that is never
  confirmed (no return-OK) self-expires, so a partial or abandoned
  setup self-heals with no distributed rollback protocol required; an
  explicit teardown/nack becomes a fast-path optimization (release in
  seconds instead of waiting out the lease) rather than a correctness
  requirement. This is the principled, first-class version of the
  ad-hoc "orphaned circuit reaped by idle/scanner cleanup" mitigation
  the reroute open questions rely on.
- **Smart reroute moves up a level**: it operates on paths, not
  circuits. Circuits ride along whichever path their circuit-record
  points at.
- **It retires Phase A's most delicate machinery and de-risks B5.** The
  C2/C3 commit-after-all-succeed + rollback discipline (Phase A concerns)
  and B5's distributed install/rollback both exist to compensate for the
  absence of independently-leased paths; Phase C removes the need for them.

**Costs:**

- **Wire protocol overhaul (the single break).** Path lifecycle messages
  (`CreatePath`, `RetainPath`, `ReleasePath`) are new; route messages
  restructure. A `routing-v2` capability gate exactly as proposed in
  `routing-v2.md`; both moves want the same gate. Design it as the target
  shape so Phase D is purely additive.
- **Forwarder data model change.** Today `circuitId -> forwardTable
  -> (srcAddr -> dstAddr)`. New model: `pathId -> pathForwards` for
  the long-haul state, plus `circuitId -> (pathId, ingressXgress,
  egressXgress)` for the endpoint binding. Two-level lookup. Possibly
  worth doing the byte-encoding cleanup from `routing-v2.md` while
  you're in there.
- **Path lifecycle.** Teardown is lease-driven soft state, with two
  distinct leases because they fire on different conditions and want
  very different durations:
  - *Setup-confirmation lease (short, seconds).* A path or hop that
    installed speculatively but never received its return-OK. Expires
    fast, on the order of the reroute grace window, so failed setups
    don't tie up resources. A generous idle-scale timeout (minutes)
    would be wrong here.
  - *Idle lease (minutes, e.g. 5m).* An established path whose
    referencing-circuit count drops to zero. Reference-counting on
    circuits plus a grace before teardown, so a path about to be reused
    isn't torn down and immediately rebuilt.
  Both generalize the QUIC "3xPTO retention after abandon" idea already
  noted below. Plus path-level fault handling and path-level metrics
  independent of any circuit. Different operational model than today.
...
**Effort:** quarter-scale. The forwarder and wire foundation is real,
bounded work, and it is separable from Phase D's authorization-delegation
research. Because its payoff (make-before-break reroute, soft-state cleanup,
retiring the C2/C3 and B5 rollback machinery) lands squarely on this
design's subject, Phase C is a strong candidate to pull forward once Phase A
has shipped.

### Phase D: Path sharing and router-local circuit creation

Builds on Phase C additively, with no second wire break:

- **Path sharing.** The reference count grows from 0/1 to 0..N: many
  circuits ride one path. Needs path-planning policy (which circuits share
  which path, disjointness and latency constraints).
- **Sub-millisecond circuit creation.** Routers create circuits *locally*
  when a path already exists to the destination and the identity is
  authorized (via RDM, the same authorization that powers V2 sessionless
  dial), with no controller round-trip on the steady-state hot path.
- **Multi-path becomes structural.** A circuit references an ordered list of
  paths trivially; the concurrent-use mechanics and per-path metrics overlap
  with B2.

**Costs:**

- **Authorization delegation.** Routers creating circuits locally must
  validate "this identity may use this service on this path" without a
  controller round-trip. RDM already provides service-level authorization;
  path-level authorization is new (does every identity get to use every
  path?). Probably paths are tagged with authorization scopes at creation.
  This is the hard, separable research Phase C deliberately defers.
- **Path planning for sharing.** Deciding which circuits coalesce onto
  which paths, under disjointness and latency constraints, is genuine
  algorithm work.
...
and the B phases with this destination in mind is to avoid wire-protocol
or token decisions that would have to be reworked when C and D land.
...
- `xgressId`: the xgress address of the endpoint this token authorizes
  taking over, for the side named by `side` (the ingress xgress id for
  an `Ingress` token, the egress xgress id for an `Egress` token). It is
  fixed at circuit creation and preserved across every reroute, so the
  new router pre-registers its forwarder at this controller-signed
  address rather than allocating one. Naming it side-scoped rather than
  `ingressId` lets B1 reuse it for the egress side with no new claim.
...
- **Multiple connection IDs per connection (RFC 9000 §5.1).** QUIC
  ...
  per-path IDs in phase A or B1. Phase B2 and D may want this;
  revisit then.
...
### What phases B2, C, and D may want from QUIC multipath
...
- **Per-path congestion control**. The xgress send buffer needs
  ...
  phase D.
- **3xPTO retention after abandon.** When a path is closed, in-flight
...
- **Token design is the load-bearing decision** for everything after
  ...
  owner), and authorizes path-attach in C and D (token grants use of
  paths in this scope). Token shape is stable across all phases: claim
  values evolve (`side`, multi-path scoping, owner role), the
  envelope doesn't.
- **Don't build the full long-lived-path model first, but Phase C is
  pullable.** The end state (Phase D's sharing and router-local creation)
  is right, but its authorization-delegation and path-planning work is
  bigger than A, B1, B2, and B3 combined, and doing it first forfeits
  reroute capability for the duration. Phase C (path/circuit separation,
  no sharing) is the separable, quarter-scale subset: it pays the single
  wire break, delivers make-before-break reroute and soft-state cleanup,
  and retires the C2/C3 and B5 rollback machinery (the Phase A concerns).
  It is scheduled right after the pre-C survivability phases (A, B1, B4). It
  must be designed as the target wire shape so Phase D is additive, not a second
  break.
- **B1 and B4 build on A and precede C; B2, B3, and B5 build on C and
  follow it.** This corrects the earlier framing that all B phases were
  independent and built only on A. B1 (terminator-side survival) and B4
  (transient controller<->router loss must not tear down circuits whose
  data plane is fine) extend A's machinery on the fused model and ship
  before C. After C: B2 (multi-path) is structural once paths are
  first-class, so it waits for C rather than being built twice on the
  fused model. B5 (source-routed install) is largely enabled by C:
  route-by-path, the path-install-plus-probe traversal, and the soft-state
  leases that dissolve its distributed install/rollback cost. B3
  (cross-controller takeover) largely folds into D, since D's router-local
  reroute removes most of its owner-outage motivation, leaving only
  cross-controller path ownership/creation, reassessed post-C alongside D
  and B5 as one decentralization track (which also pairs with the gossip
  work for routing in a partial mesh).
...
## Minimally viable surface (backward compatibility)

Phase A ships as one unit; the concern here is different. Phase A is the *first release of this
protocol surface*, and everything after it (B1-B5, C, D) builds on what A puts on the wire. So the
minimally viable surface is whatever Phase A must get right up front so the later phases land
additively, without a backward-compat break for SDKs, routers, or controllers already running the
A-era protocol.

Rule of thumb: the compat-sensitive surface is the token envelope, the capability bits, and the
`version` claim. Those are the evolution machinery. Get them right in A and later phases change
claim *values*, add *capability-gated* features, or (for C) take one deliberate, gated wire break,
none of which reworks what A shipped.

How each later phase rides on A's surface:

- **B1 (terminator-side):** additive. Uses the token's `side` claim (already in A) with
  `side: Egress`; same takeover message and handler.
- **B2 (multi-path):** mostly additive. Adds an `AdditionalPath` flag to the takeover request (a
  new header) and per-path identity. A must not bake replacement-only iteration semantics into
  anything a peer *requires*; the `iteration` claim is fine, the additional-vs-replacement
  decision is deferred (see Phase B2).
- **B3 (cross-controller):** additive behind its own capability bit. Adds `(ownerId, iteration)`
  to route/unroute/fault messages; A's `ownerControllerId` claim becomes a hint rather than a
  constraint. No A rework.
- **B4 (presence grace):** controller-model behavior only, no SDK-facing wire. Additive.
- **B5 (source-routed install):** additive behind its own capability bit; reuses A's token
  signing/verification machinery.
- **C (path/circuit separation):** the one deliberate wire break, gated by a `routing-v2`
  capability exactly as A gates itself. A's token is designed to carry through (it authorizes
  path-attach in C); the envelope is stable, only claim usage evolves.
- **D:** additive on C.

Items to settle before A's first release (TBD, to be finalized once the skeleton is implemented
and the true shape is settled):

- **SDK-signed takeover proof.** The one to watch. If the controller will ever *require* a signed
  proof (dropping trust in the router's identity attestation, see the threat model), then A must
  either ship it or reserve the gate for it. The `version` claim plus the reroute-capability bit
  are the intended hedge: a "signed-takeover-required" mode can be gated on them and enabled only
  among capable peers, so requiring it later is not a flag day. Decide whether that gating is
  enough or the proof should be mandatory in A.
- **TakeoverResult codes (decided).** The SDK treats an unknown result code as fatal-abort, so new
  codes stay additive. This is a Phase A requirement of the recovery loop, not a later add; every
  future `TakeoverResult` value an older SDK does not recognize simply surfaces the conn closed.
- **Endpoint address is a side-scoped claim (decided).** The taken-over endpoint's xgress address
  rides the token as `xgressId`, scoped by the `side` claim (ingress xgress id for an Ingress
  token, egress xgress id for an Egress token) rather than a fixed `ingressId`. B1's egress-side
  takeover then reuses it verbatim with `side: Egress`, with no new claim and no version bump. The
  general rule stands: any claim a later phase will *require* (not just read opportunistically)
  must exist or be version-gated from A. `side` and `xgressId` are the endpoint-scoping pair.

Release timing: once A's protocol surface ships in a release it is committed, deployed peers speak
it, and evolution is bound by the rules above. Because A introduces a non-trivial surface (a
capability bit, the reroute token, new content types, the `FaultReason` field), we may prefer to
merge A *after* the 2.1 release rather than mid-cycle, so we are not locked into the surface before
it has burned in. TBD alongside the required-in-A analysis.
...
- **Lost takeover success reply (accepted for Phase A).** If a
  ...
  its speculative registrations on close. (Phase C's soft-state path
  leases are the principled version of this reaping: the orphaned
  path/circuit state self-expires on its idle lease rather than relying
  on scanner cleanup to notice it.) No corruption, but a
...
- draft-ietf-quic-multipath: explicit path identifiers, PATH_ABANDON,
  per-path congestion control; informs phases B2, C, and D.
