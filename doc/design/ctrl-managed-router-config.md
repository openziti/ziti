...
1. **A router's `Configs` list changes.**
   ...
   - Whenever the router's own `Router` event flows (any create or update, not just the initial
     association), also emit the full `Config` entity for *every* config in its list, as synthetic
     `Config` `Create` events. The receiver caches and dedupes, so re-sending the whole list beats
     tracking which IDs are newly added. (Full syncs skip this; the snapshot already carries every
     `Config`.)
...
Router entities themselves (id, name, fingerprint, configs, disabled) broadcast to all routers so
every router can validate link peers.
...
`RouterSender` forwards every change set even when filtering empties it, so an empty change set
advances the receiver's index and the `PreviousIndex` chain stays intact without per-router
bookkeeping. (Tracking the previous index per router instead proved racy under subscription churn
and produced gap-detection storms.)
...
controller-loaded config. For `router.link.v1`: rename `dialer.bind` → `dialer.bindInterface`,
default `binding` if absent, and preserve the deprecated `split` dialer flag as a local-only field
...
- `preserve` (default): never auto-act on locally-stale links.
...
`gcMode` scopes **local** staleness only, meaning links this router's own configuration has
invalidated. It does not gate peer-driven closure: when a peer withdraws or moves a listener, the
resulting `PeerStateChange` closes the affected link immediately under every mode, including
`preserve`. That path predates auto-GC and is deliberately left ungated, because the peer's
advertised listener set is authoritative about what it will accept, and because it is what
re-homes a link when a peer rebinds. Dialer-side staleness compares binding names rather than
addresses, so gating it would strand the old link with no check able to reclaim it. Operators
should read `preserve` as "this router won't GC on its own config changes", not as "links never
close on a config change".
