...
2. Upgrade the controller to the "to" version (default: v2.0.0). This auto-enables OIDC.
...
The existing `zititest/models/smoke` model already has the exact topology we want (ERT, ZET, and
ziti-tunnel clients and hosts, plus a Go SDK loop4 sim). The `backward-compatibility.yml` workflow
already runs that model with mixed old/new binary versions. But the smoke model decides HA-vs-standalone
at bootstrap and has fast one-shot pass/fail semantics. The upgrade test needs dynamic, mid-run version
swaps, an in-place standalone->HA conversion, and continuous traffic across the transitions. Rather than
complicate smoke (and risk the compat runs that depend on it), we build a new model,
`zititest/models/upgrade-test`, that copies smoke's topology and borrows the continuous-sim harness from
`circuit-test` / `private-ctrl-test`.
...
Support components (single, not paired): plain-TCP loop4 backends behind each tunneler host
(`loop4-ert-backend` on the ERT host, `loop4-zet-backend`, `loop4-zt-backend`) that the ZET/ziti-tunnel/ERT
hosts forward their loop service to (see Liveness). These have no ziti identity, so they are not part of
the OIDC story.

## Version parameterization

Both ends are fully parameterized (version X -> version Y), defaulting to latest 1.6.x -> v2.0.0.

- `fromVersion` / `toVersion` model variables drive the controller and router binaries.
- `zetVersion` / `zetHostVersion` for ZET. ZET is not upgraded mid-run; it stays up the whole time.
  Parameterizing it lets us sweep ZET versions across separate runs. Default is the version shipped with
  the current stable Windows desktop edge client: `v1.18.0` (bundled in Ziti Desktop Edge for Windows
  release 2.11.2.5, the latest stable release; note 2.11.2.7 bundles v1.18.2 but is a pre-release).
...
6. Final validation: full clean steady-state, and assert the *un-restarted* instance of each pair kept
   its original PID across phases 1-5 (proves the long-lived clients truly stayed up).
...
An outer loop action runs `{reset -> full upgrade sequence}` for `iterations` runs. Rather than
hand-rolling a surgical state wipe, reset leans on the existing fablab machinery: wipe everything
(controller DB and raft state, PKI/enrollment, cached tokens) and re-run distribution and bootstrap from
the `fromVersion` standalone configuration. This is more trustworthy than trying to selectively clear
state, and an incomplete wipe would produce cross-iteration contamination that looks exactly like a race.
...
`iterations` defaults to 1 so a normal run is bounded; the loop is opt-in for long local soak runs and a
time-boxed slot in the validation suite.

## Delivery

- Model directory `zititest/models/upgrade-test` with `main.go`, `bootstrap.go`, `configs/`, reusing the
  `zitilab` sim harness.
- A `workflow_dispatch` GitHub workflow mirroring `backward-compatibility.yml`, matrixed over ZET
  versions and from/to versions.

## Items to verify during implementation

- Router controller-endpoint discovery after HA conversion (config vs auto-learn).
- Exact `ziti agent cluster add` invocation and the three-node quorum readiness check.
- Running loop4 as a plain-TCP listener behind the tunnel hosts for the ZET and ziti-tunnel hosted
  services.
- Config re-render and rsync flow for injecting the `cluster:` directive mid-run.

## Implementation order

Build incrementally, one phase at a time:

1. From-version topology plus the continuous sim, green at steady state. (done, single instances)
1b. Pairing enhancement: add the second instance of each SDK client/host (co-located), split
    `loop4-host` into a paired `loop4-sdk-host` plus a single `loop4-ert-backend`, add the config id to
    `Loop4SimType`'s process filter, and enable the `apiSession` event in `ctrl.yml.tmpl`.
2. Controller upgrade phase, plus the restart-one-of-each OIDC verification (read `apiSession` type). (done)
   Actions: `upgradeController` (pre-staged toVersion binary, version swap + restart), `restartAndVerifyOidc`
   (restart `.restart` instances, read new `apiSession` `created` events from the controller log, require
   Go SDK clients to come back `jwt`, record ZET), and a `phase2` composite that brackets them with the
   steady-state gate.
3. Rolling router upgrade phase.
4. HA conversion phase.
5. Add-nodes phase.
6. Iteration loop and the GitHub workflow.
