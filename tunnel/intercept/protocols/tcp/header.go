...
func (b TCP) CalculateChecksum(partialChecksum uint16, totalLen uint16) uint16 {
	...
	checksum := ip.Checksum(tmp, partialChecksum)

	// Calculate the rest of the checksum.
	return ip.Checksum(b[:b.DataOffset()], checksum)
}
...
func (b TCP) EncodePartial(partialChecksum, length uint16, seqnum, acknum uint32, flags byte, rcvwnd uint16) {
	...
	checksum = ip.Checksum(b[seqNum:seqNum+8], checksum)
	checksum = ip.Checksum(b[winSize:winSize+2], checksum)
